...
// ═══════════════════════════════
function fW(v){return Math.round(v||0).toLocaleString();}
...
[TraidAIr 상태 진단]
━━━━━━━━━━━━━━━━━━━━━━
💰 자본: 현금 ${fW(mock.cash)}원 / 보유 ${Object.keys(mock.positions||{}).length}종목
...
function _roundTick(p){
  ...
  if(p>=20000)return Math.round(p/50)*50;
...
function _peekSimCandlesFor(tk, dateStr, untilIdx){
  try{
    const stk = (typeof STOCKS!=='undefined' ? STOCKS : []).find(s=>s.tk===tk);
    if(!stk) return [];
    ...
    const genDay = (dayStr, isPrev) => {
      ...
      for(let i=0;i<perDay;i++){
        ...
        out.push({t:`${String(Math.floor(hm/60)).padStart(2,'0')}:${String(hm%60).padStart(2,'0')}`,date:dayStr,isPrev:!!isPrev,o:_roundTick(o),h:_roundTick(Math.max(o,c,h)),l:_roundTick(Math.min(o,c,l)),c:_roundTick(c),v});
        base = c;
      }
      return out;
    };
...
function renderPending(){const el=document.getElementById("pendingList");if(!el)return;if(!pendingOrders.length){el.innerHTML="<div style='font-size:9px;color:var(--tm);padding:8px;'>미체결 없음</div>";return;}el.innerHTML=pendingOrders.map(o=>`<div style="padding:4px 8px;border-bottom:1px solid var(--br);display:flex;align-items:center;gap:4px;"><span style="font-size:9px;font-weight:700;color:${o.side==="buy"?"var(--g)":"var(--r)"};">${o.side==="buy"?"매수":"매도"}</span><span style="font-size:9px;flex:1;">${o.nm} ${fW(o.price)}×${o.qty}</span><span style="font-size:8px;color:var(--tm);">${o.time}</span><button onclick="cancelPending(${o.id})" style="background:var(--r);color:#fff;border:none;border-radius:3px;font-size:8px;padding:1px 4px;cursor:pointer;">취소</button></div>`).join("");}
...
function _renderRealOrderbook(d){
  ...
  [...asks].reverse().forEach(a => {
    ...
    sell += `<div class="ob-r" onclick="document.getElementById('ofPr').value=${a.price};updOSum()"><div class="ob-bg s" style="width:${w}%"></div><div class="ob-p" style="color:#2563eb;">${fW(a.price)}</div><div class="ob-q">${a.qty.toLocaleString()}</div></div>`;
  });

  let buy = '';
  bids.forEach(b => {
    const w = Math.min(100, (b.qty/maxQ*100)).toFixed(0);
    buy += `<div class="ob-r" onclick="document.getElementById('ofPr').value=${b.price};updOSum()"><div class="ob-bg b" style="width:${w}%"></div><div class="ob-p" style="color:#dc2626;">${fW(b.price)}</div><div class="ob-q">${b.qty.toLocaleString()}</div></div>`;
  });
...
function _genSimCandles(tk,dt){
  ...
  if(sim.tf==="D"||sim.tf.startsWith("D")){
    ...
    while(res.length<DAYS){
      ...
      res.push({t:ds,o:_roundTick(o),h:_roundTick(h),l:_roundTick(l),c:_roundTick(c),v});
      base=c;
    }
    sim.candles=res;
...
// ═══════════════════════════════════════════════
// 진화 엔진: 성과 프로파일 + 적응형 파라미터
// ═══════════════════════════════════════════════
var _perfProfile={};
var _adaptiveCfg={stop:3,t1:3,t2:5,pos:30,rr:1.5,minScore:6};
// 종합 점수: 승률(40) + 손익비(30) + 수익팩터(30) = 0~100
function _calcScore(wr,rr,pf){
  var wrPart=Math.min(40,wr*0.4);
  var rrPart=Math.min(30,Math.max(0,(parseFloat(rr)||0))*15);
  var pfPart=Math.min(30,Math.max(0,(pf||0))*10);
  return Math.round(wrPart+rrPart+pfPart);
}
function computePerformanceProfile(){
  var trades=mock.trades||[];
  var sells=trades.filter(function(t){return t.side==='sell';});
  if(sells.length<3) return _perfProfile;
  var byTech={};
  sells.forEach(function(t){
    var tech=t.technique||'기법없음';
    if(!byTech[tech]) byTech[tech]={wins:0,losses:0,totalPnl:0,count:0,avgWin:0,avgLoss:0,pnls:[],streak:0,maxStreak:0};
    var b=byTech[tech];
    b.count++;b.totalPnl+=(t.pnl||0);b.pnls.push(t.pnl||0);
    if((t.pnl||0)>0){b.wins++;b.streak++;if(b.streak>b.maxStreak)b.maxStreak=b.streak;}
    else{b.losses++;b.streak=0;}
  });
  Object.keys(byTech).forEach(function(k){
    var b=byTech[k];
    var ws=b.pnls.filter(function(p){return p>0;});
    var ls=b.pnls.filter(function(p){return p<=0;});
    b.winRate=b.count?Math.round(b.wins/b.count*100):0;
    b.avgWin=ws.length?Math.round(ws.reduce(function(a,c){return a+c;},0)/ws.length):0;
    b.avgLoss=ls.length?Math.round(ls.reduce(function(a,c){return a+Math.abs(c);},0)/ls.length):1;
    b.rr=b.avgLoss>0?(b.avgWin/b.avgLoss).toFixed(2):'--';
    var totalLoss=ls.reduce(function(a,c){return a+Math.abs(c);},0);
    b.pf=totalLoss>0?parseFloat((ws.reduce(function(a,c){return a+c;},0)/totalLoss).toFixed(2)):0;
    b.score=_calcScore(b.winRate,b.rr,b.pf);
    b.grade=b.score>=70?'A':b.score>=50?'B':b.score>=30?'C':'D';
    b.kelly=b.winRate>0&&b.avgLoss>0?Math.max(0,((b.winRate/100)-(1-b.winRate/100)/(b.avgWin/(b.avgLoss||1)))*100).toFixed(1):'0';
  });
  var recent20=sells.slice(-20);
  var recentWr=recent20.length?Math.round(recent20.filter(function(t){return(t.pnl||0)>0;}).length/recent20.length*100):50;
  var recentAvgWin=0,recentAvgLoss=0;
  var rw=recent20.filter(function(t){return(t.pnl||0)>0;});
  var rl=recent20.filter(function(t){return(t.pnl||0)<=0;});
  if(rw.length)recentAvgWin=Math.round(rw.reduce(function(a,t){return a+(t.pnl||0);},0)/rw.length);
  if(rl.length)recentAvgLoss=Math.round(rl.reduce(function(a,t){return a+Math.abs(t.pnl||0);},0)/rl.length);
  var recentRr=recentAvgLoss>0?(recentAvgWin/recentAvgLoss):1;
  var recentTotalLoss=rl.reduce(function(a,t){return a+Math.abs(t.pnl||0);},0);
  var recentPf=recentTotalLoss>0?(rw.reduce(function(a,t){return a+(t.pnl||0);},0)/recentTotalLoss):0;
  var recentScore=_calcScore(recentWr,recentRr,recentPf);
  _perfProfile={byTech:byTech,totalSells:sells.length,recentWr:recentWr,recentAvgWin:recentAvgWin,recentAvgLoss:recentAvgLoss,recentScore:recentScore,recentRr:parseFloat(recentRr.toFixed(2)),recentPf:parseFloat(recentPf.toFixed(2)),updated:Date.now()};
  try{localStorage.setItem('htsPerfProfile',JSON.stringify(_perfProfile));}catch(_e){}
  computeAdaptiveConfig();
  return _perfProfile;
}
function computeAdaptiveConfig(){
  var p=_perfProfile;if(!p.totalSells||p.totalSells<5)return;
  var sc=p.recentScore||50;
  var wr=p.recentWr||50;
  var rr=p.recentRr||1;
  // 점수 기반 적응형 파라미터
  _adaptiveCfg.stop=sc>=70?3.5:sc>=50?3:sc>=30?2.5:2;
  _adaptiveCfg.t1=Math.max(2,Math.round(_adaptiveCfg.stop*(rr>=1.5?1.5:rr>=1?1.2:1)*10)/10);
  _adaptiveCfg.t2=Math.round(_adaptiveCfg.t1*1.7*10)/10;
  var kellyPct=(wr/100)-((1-wr/100)/(rr||1));
  var halfKelly=Math.max(10,Math.min(50,Math.round(kellyPct*50)));
  _adaptiveCfg.pos=sc>=70?Math.min(50,halfKelly+10):sc>=50?halfKelly:sc>=30?Math.max(10,halfKelly-5):10;
  _adaptiveCfg.rr=sc>=70?1.2:sc>=50?1.5:2.0;
  _adaptiveCfg.minScore=sc>=70?5:sc>=50?6:sc>=30?7:8;
  _adaptiveCfg.score=sc;
  console.log('[적응형]','점수:'+sc+'/100','stop:'+_adaptiveCfg.stop+'%','pos:'+_adaptiveCfg.pos+'%','rr:'+_adaptiveCfg.rr,'minScore:'+_adaptiveCfg.minScore);
}
function getTechGrade(techName){
  if(!_perfProfile.byTech)return{grade:'?',score:50,adjust:1};
  var t=_perfProfile.byTech[techName];
  if(!t||t.count<3)return{grade:'?',score:50,adjust:1};
  var adj=t.grade==='A'?1.3:t.grade==='B'?1.0:t.grade==='C'?0.7:0.4;
  return{grade:t.grade,score:t.score,winRate:t.winRate,rr:t.rr,pf:t.pf,count:t.count,kelly:t.kelly,adjust:adj};
}
function buildPerfSummary(){
  if(!_perfProfile.byTech)return'성과 데이터 없음';
  var lines=[];
  Object.keys(_perfProfile.byTech).forEach(function(k){
    var t=_perfProfile.byTech[k];
    if(t.count<2)return;
    lines.push(k+': '+t.grade+'등급 '+t.score+'점 승률'+t.winRate+'% R/R'+t.rr+' PF'+t.pf+' '+t.count+'건');
  });
  lines.push('최근20건 종합점수: '+(_perfProfile.recentScore||'-')+'/100');
  lines.push('적응형 손절: '+_adaptiveCfg.stop+'% / 비중: '+_adaptiveCfg.pos+'% / 최소R/R: '+_adaptiveCfg.rr);
  return lines.join('\n');
}
try{var _sp=localStorage.getItem('htsPerfProfile');if(_sp)_perfProfile=JSON.parse(_sp);computeAdaptiveConfig();}catch(_e){}

// ═══════════════════════════════════════════════
// AI 학습 엔진 v2: 패턴 DB + 원칙 규칙 + 준수 추적 + 가중치 진화 + 시장 적응
// ═══════════════════════════════════════════════

// ── 1. 승/패 조건 DB (매 거래의 진입 시점 조건 기록 + 결과 매핑) ──
var _patternDB=[];
try{var _pd=localStorage.getItem('htsPatternDB');if(_pd)_patternDB=JSON.parse(_pd);}catch(_e){}
function captureEntryConditions(){
  var cs=sim.candles;if(!cs||sim.idx<10)return null;
  var cls=cs.slice(0,sim.idx+1).map(function(c){return c.c;});
  var vls=cs.slice(0,sim.idx+1).map(function(c){return c.v;});
  var lc=cs[sim.idx],pc=cs[sim.idx-1]||lc;
  var ma5=calcMA(cls,5),ma20=calcMA(cls,20);
  var lma5=ma5[ma5.length-1]||0,lma20=ma20[ma20.length-1]||0;
  var rsi=parseFloat((calcRSI(cls,14).slice(-1)[0]||50).toFixed(0));
  var avg5v=vls.length>=6?(vls.slice(-6,-1).reduce(function(a,b){return a+b;},0)/5):vls[vls.length-2]||1;
  var volR=parseFloat((vls[vls.length-1]/avg5v).toFixed(1));
  var body=Math.abs(lc.c-lc.o),range=lc.h-lc.l||1;
  var _hm=String(lc.t||'').split(':'),hour=parseInt(_hm[0])||9;
  return{
    maAlign:lma5>lma20?1:0,
    priceVsMa5:lma5>0?parseFloat(((lc.c-lma5)/lma5*100).toFixed(1)):0,
    rsi:rsi,volR:volR,
    bodyRatio:parseFloat((body/range).toFixed(2)),
    isBull:lc.c>lc.o?1:0,
    prevBull:pc.c>pc.o?1:0,
    hour:hour,
    spread:parseFloat(((lc.h-lc.l)/lc.c*100).toFixed(2))
  };
}
function recordPattern(side,technique,conditions,pnl){
  if(!conditions)return;
  var entry={date:sim.date,side:side,tech:technique,cond:conditions,pnl:Math.round(pnl||0),win:pnl>0?1:0,ts:Date.now()};
  _patternDB.push(entry);
  if(_patternDB.length>1000)_patternDB=_patternDB.slice(-1000);
  try{localStorage.setItem('htsPatternDB',JSON.stringify(_patternDB));}catch(_e){}
}
function queryPatternDB(conditions,technique){
  if(_patternDB.length<10)return{score:0,count:0,msg:'데이터부족'};
  var similar=_patternDB.filter(function(p){
    if(technique&&p.tech!==technique)return false;
    var c=p.cond;if(!c)return false;
    var match=0,total=0;
    if(c.maAlign===conditions.maAlign){match++;}total++;
    if(Math.abs(c.rsi-conditions.rsi)<=10){match++;}total++;
    if(Math.abs(c.volR-conditions.volR)<=0.5){match++;}total++;
    if(c.isBull===conditions.isBull){match++;}total++;
    if(c.hour===conditions.hour){match++;}total++;
    return match>=3;
  });
  if(similar.length<3)return{score:0,count:similar.length,msg:'유사패턴부족'};
  var wins=similar.filter(function(p){return p.win;}).length;
  var wr=Math.round(wins/similar.length*100);
  var avgPnl=Math.round(similar.reduce(function(a,p){return a+p.pnl;},0)/similar.length);
  return{score:wr,count:similar.length,avgPnl:avgPnl,msg:wr+'%승률('+similar.length+'건) 평균'+avgPnl+'원'};
}

// ── 2. 강의 원칙 → 하드 규칙 (텍스트가 아닌 코드 게이트) ──
var LECTURE_RULES=[
  {name:'눌림목_MA지지',tech:'눌림목',check:function(c,lma5,lma20){return c.c>=lma20&&Math.abs(c.c-lma5)/lma5<=0.02;},msg:'20MA 위 + 5MA 근접'},
  {name:'돌파_거래량',tech:'돌파',check:function(c,lma5,lma20,volR){return volR>=1.5;},msg:'거래량 150%+ 필수'},
  {name:'돌파_신고가',tech:'돌파',check:function(c,lma5,lma20,volR,prev20High){return c.c>prev20High*0.998;},msg:'20봉 신고가 근접'},
  {name:'갭상승_양봉확정',tech:'갭상승',check:function(c){return c.c>c.o;},msg:'갭 후 양봉 필수'},
  {name:'첫봉_시초6봉',tech:'첫봉',check:function(c,lma5,lma20,volR,p20h,idx,prevCount){return(idx-prevCount)<=6;},msg:'시초 6봉 이내'},
  {name:'공통_RSI과매수금지',tech:'*',check:function(c,lma5,lma20,volR,p20h,idx,prevCount,rsi){return rsi<72;},msg:'RSI 72 미만'},
  {name:'공통_추격금지',tech:'*',check:function(c,lma5,lma20,volR,p20h,idx,prevCount,rsi,pc){return!pc||c.c<=pc.c*1.03;},msg:'전봉대비 +3% 이내'},
  {name:'공통_3연음봉금지',tech:'*',check:function(c,lma5,lma20,volR,p20h,idx,prevCount,rsi,pc,pc2){return!(c.c<c.o&&pc&&pc.c<pc.o&&pc2&&pc2.c<pc2.o);},msg:'3연속 음봉 아님'}
];
function checkLectureRules(tech,c,lma5,lma20,volR,prev20High,idx,prevCount,rsi,pc,pc2){
  var passed=0,failed=0,failMsgs=[];
  LECTURE_RULES.forEach(function(rule){
    if(rule.tech!=='*'&&rule.tech!==tech)return;
    try{
      if(rule.check(c,lma5,lma20,volR,prev20High,idx,prevCount,rsi,pc,pc2)){passed++;}
      else{failed++;failMsgs.push(rule.name+': '+rule.msg);}
    }catch(_e){passed++;}
  });
  return{passed:passed,failed:failed,total:passed+failed,rate:passed+failed>0?Math.round(passed/(passed+failed)*100):100,failMsgs:failMsgs};
}

// ── 3. 원칙 준수 추적 (거래마다 기록, 준수율↔수익률 상관관계) ──
var _adherenceLog=[];
try{var _al=localStorage.getItem('htsAdherenceLog');if(_al)_adherenceLog=JSON.parse(_al);}catch(_e){}
function recordAdherence(tech,ruleResult,pnl){
  _adherenceLog.push({date:sim.date,tech:tech,rate:ruleResult.rate,passed:ruleResult.passed,total:ruleResult.total,pnl:Math.round(pnl||0),win:pnl>0?1:0,ts:Date.now()});
  if(_adherenceLog.length>500)_adherenceLog=_adherenceLog.slice(-500);
  try{localStorage.setItem('htsAdherenceLog',JSON.stringify(_adherenceLog));}catch(_e){}
}
function getAdherenceInsight(){
  if(_adherenceLog.length<10)return{highAdherenceWr:0,lowAdherenceWr:0,msg:'데이터부족'};
  var high=_adherenceLog.filter(function(a){return a.rate>=80;});
  var low=_adherenceLog.filter(function(a){return a.rate<60;});
  var highWr=high.length>=3?Math.round(high.filter(function(a){return a.win;}).length/high.length*100):0;
  var lowWr=low.length>=3?Math.round(low.filter(function(a){return a.win;}).length/low.length*100):0;
  return{highAdherenceWr:highWr,lowAdherenceWr:lowWr,highCount:high.length,lowCount:low.length,
    msg:'원칙준수(80%+) 승률'+highWr+'%('+high.length+'건) vs 미준수(<60%) 승률'+lowWr+'%('+low.length+'건)'};
}

// ── 4. 조건 가중치 자동 진화 (어떤 지표가 수익에 가장 기여하는지) ──
var _featureWeights={maAlign:1,rsi:1,volR:1,isBull:1,prevBull:1,bodyRatio:1,hour:1};
try{var _fw=localStorage.getItem('htsFeatureWeights');if(_fw)_featureWeights=JSON.parse(_fw);}catch(_e){}
function evolveFeatureWeights(){
  if(_patternDB.length<20)return;
  var wins=_patternDB.filter(function(p){return p.win;});
  var losses=_patternDB.filter(function(p){return!p.win;});
  if(wins.length<5||losses.length<5)return;
  var features=['maAlign','rsi','volR','isBull','prevBull','bodyRatio'];
  features.forEach(function(f){
    var wAvg=wins.reduce(function(a,p){return a+(p.cond[f]||0);},0)/wins.length;
    var lAvg=losses.reduce(function(a,p){return a+(p.cond[f]||0);},0)/losses.length;
    var diff=wAvg-lAvg;
    if(f==='rsi'){_featureWeights[f]=diff>3?1.2:diff<-3?0.8:1;}
    else if(f==='volR'){_featureWeights[f]=diff>0.3?1.3:diff<-0.3?0.7:1;}
    else{_featureWeights[f]=diff>0.1?1.2:diff<-0.1?0.8:1;}
  });
  // 시간대별 승률
  var hourWr={};
  _patternDB.forEach(function(p){
    var h=p.cond.hour||9;
    if(!hourWr[h])hourWr[h]={w:0,t:0};
    hourWr[h].t++;if(p.win)hourWr[h].w++;
  });
  var bestHour=9,bestHourWr=0;
  Object.keys(hourWr).forEach(function(h){
    var wr=hourWr[h].t>=5?hourWr[h].w/hourWr[h].t:0;
    if(wr>bestHourWr){bestHourWr=wr;bestHour=parseInt(h);}
  });
  _featureWeights.bestHour=bestHour;
  _featureWeights.hourWr=hourWr;
  try{localStorage.setItem('htsFeatureWeights',JSON.stringify(_featureWeights));}catch(_e){}
}
function applyFeatureWeights(score,conditions){
  if(!conditions)return score;
  var bonus=0;
  if(conditions.maAlign&&_featureWeights.maAlign>1)bonus+=1;
  if(conditions.volR>=1.5&&_featureWeights.volR>1)bonus+=1;
  if(conditions.isBull&&_featureWeights.isBull>1)bonus+=1;
  if(conditions.hour===_featureWeights.bestHour)bonus+=1;
  if(_featureWeights.hourWr){
    var hw=_featureWeights.hourWr[conditions.hour];
    if(hw&&hw.t>=5&&hw.w/hw.t<0.3)bonus-=1;
  }
  return score+bonus;
}

// ── 5. 시장 상태 적응 (상승/하락/박스권 감지 + 기법별 상태 성과) ──
function detectMarketRegime(){
  var cs=sim.candles;if(!cs||cs.length<40)return'unknown';
  var cls=cs.slice(0,Math.min(sim.idx+1,cs.length)).map(function(c){return c.c;});
  if(cls.length<20)return'unknown';
  var ma20=calcMA(cls,20);
  var recent=ma20.slice(-10).filter(function(v){return v!==null;});
  if(recent.length<5)return'unknown';
  var rising=0,falling=0;
  for(var i=1;i<recent.length;i++){
    if(recent[i]>recent[i-1])rising++;else falling++;
  }
  if(rising>=7)return'uptrend';
  if(falling>=7)return'downtrend';
  return'sideways';
}
var _regimePerf={};
try{var _rp=localStorage.getItem('htsRegimePerf');if(_rp)_regimePerf=JSON.parse(_rp);}catch(_e){}
function recordRegimePerf(regime,technique,pnl){
  var key=regime+'_'+technique;
  if(!_regimePerf[key])_regimePerf[key]={w:0,l:0,pnl:0};
  _regimePerf[key].pnl+=pnl;
  if(pnl>0)_regimePerf[key].w++;else _regimePerf[key].l++;
  try{localStorage.setItem('htsRegimePerf',JSON.stringify(_regimePerf));}catch(_e){}
}
function getRegimeAdvice(regime,technique){
  var key=regime+'_'+technique;
  var r=_regimePerf[key];
  if(!r||r.w+r.l<3)return{adj:1,msg:''};
  var wr=Math.round(r.w/(r.w+r.l)*100);
  if(wr>=60)return{adj:1.1,msg:regime+'+'+technique+' 승률'+wr+'%↑'};
  if(wr<=30)return{adj:0.8,msg:regime+'+'+technique+' 승률'+wr+'%↓'};
  return{adj:1,msg:''};
}

// ── 6. 자본곡선 드로다운 서킷브레이커 ──
var _equityCurve={peakCash:0,maxDDPct:0,ddStartTime:null,tierHit:0};
function updateEquityCurve(){
  var equity=mock.cash;
  Object.entries(mock.positions||{}).forEach(function(e){
    var pos=e[1];if(!pos||pos.qty<=0)return;
    var stk=STOCKS.find(function(s){return s.tk===e[0];});
    if(stk)equity+=pos.qty*stk.pr;
  });
  if(equity>_equityCurve.peakCash)_equityCurve.peakCash=equity;
  if(_equityCurve.peakCash<=0)_equityCurve.peakCash=equity;
  var ddPct=_equityCurve.peakCash>0?((_equityCurve.peakCash-equity)/_equityCurve.peakCash*100):0;
  if(ddPct>_equityCurve.maxDDPct)_equityCurve.maxDDPct=ddPct;
  if(ddPct>=3){_equityCurve.tierHit=3;}
  else if(ddPct>=2){_equityCurve.tierHit=2;}
  else if(ddPct>=1){_equityCurve.tierHit=1;if(!_equityCurve.ddStartTime)_equityCurve.ddStartTime=Date.now();}
  else{_equityCurve.tierHit=0;_equityCurve.ddStartTime=null;}
  if(_equityCurve.tierHit===1&&_equityCurve.ddStartTime&&Date.now()-_equityCurve.ddStartTime>2700000)_equityCurve.tierHit=2;
  return _equityCurve;
}
function resetEquityCurve(){_equityCurve.peakCash=mock.cash;_equityCurve.maxDDPct=0;_equityCurve.ddStartTime=null;_equityCurve.tierHit=0;}

// ── 7. 다중신호 청산 최적화 (모멘텀 소진/꼬리/다이버전스 감지) ──
function computeExitSignal(tk){
  var pos=mock.positions[tk];var so=stopOrders[tk];
  if(!pos||!so)return null;
  var cs;try{cs=tk===activeTk?sim.candles.slice(0,sim.idx+1):_peekSimCandlesFor(tk,sim.date,sim.idx);}catch(_e){return null;}
  if(!cs||cs.length<10)return null;
  var cls=cs.map(function(c){return c.c;});var vls=cs.map(function(c){return c.v;});
  var lc=cs[cs.length-1];var urgency=0;var reasons=[];
  // 거래량 이탈 (가격↑ 거래량↓)
  if(cls.length>=4&&cls[cls.length-1]>cls[cls.length-3]&&vls[cls.length-1]<vls[cls.length-3]*0.7){urgency+=2;reasons.push('거래량이탈');}
  // 윗꼬리 비대
  var body=Math.abs(lc.c-lc.o);var upperW=lc.h-Math.max(lc.c,lc.o);
  if(body>0&&upperW>body*1.5){urgency+=2;reasons.push('윗꼬리비대');}
  // RSI 베어리쉬 다이버전스
  var rsiArr=calcRSI(cls,14);
  if(rsiArr.length>=4&&cls[cls.length-1]>cls[cls.length-4]&&(rsiArr[rsiArr.length-1]||50)<(rsiArr[rsiArr.length-4]||50)-3){urgency+=3;reasons.push('RSI다이버전스');}
  // 5MA 데드크로스
  var ma5=calcMA(cls,5);var ma20=calcMA(cls,20);
  if(ma5.length>=2&&ma20.length>=1&&(ma5[ma5.length-2]||0)>(ma20[ma20.length-1]||0)&&(ma5[ma5.length-1]||0)<=(ma20[ma20.length-1]||0)){urgency+=4;reasons.push('5MA데드크로스');}
  // 모멘텀 소진 (3봉 연속 축소)
  if(cs.length>=4){
    var b1=Math.abs(cs[cs.length-3].c-cs[cs.length-3].o),b2=Math.abs(cs[cs.length-2].c-cs[cs.length-2].o),b3=Math.abs(cs[cs.length-1].c-cs[cs.length-1].o);
    if(b1>b2&&b2>b3&&b3<b1*0.4){urgency+=2;reasons.push('모멘텀소진');}
  }
  if(urgency>=5)return{action:'EXIT_ALL',urgency:urgency,reasons:reasons};
  if(urgency>=3)return{action:'TIGHTEN_STOP',urgency:urgency,reasons:reasons,newStop:Math.round((so.stop+lc.c)/2)};
  return{action:'HOLD',urgency:urgency,reasons:reasons};
}

// ── 8. 순차 패턴 체인 (A 이후 B가 어떻게 되는가 + 시간감쇠) ──
var _patternChains=[];
try{var _pc2=localStorage.getItem('htsPatternChains');if(_pc2)_patternChains=JSON.parse(_pc2);}catch(_e){}
function recordPatternChain(sellTrade){
  var recent=(mock.trades||[]).filter(function(t){return t.tk===sellTrade.tk&&t.side==='sell'&&t.ts!==sellTrade.ts&&sellTrade.ts-t.ts<7200000&&sellTrade.ts-t.ts>0;});
  if(!recent.length)return;
  var prev=recent.sort(function(a,b){return b.ts-a.ts;})[0];
  _patternChains.push({date:sim.date,tk:sellTrade.tk,prevTech:prev.technique||'?',prevWin:(prev.pnl||0)>0?1:0,curTech:sellTrade.technique||'?',curWin:(sellTrade.pnl||0)>0?1:0,ts:Date.now()});
  if(_patternChains.length>500)_patternChains=_patternChains.slice(-500);
  try{localStorage.setItem('htsPatternChains',JSON.stringify(_patternChains));}catch(_e){}
}
function queryPatternChain(prevTech,prevWin){
  if(_patternChains.length<10)return{score:0,msg:''};
  var now=Date.now();var HL=30*86400000;
  var m=_patternChains.filter(function(c){return c.prevTech===prevTech&&c.prevWin===prevWin;});
  if(m.length<3)return{score:0,msg:''};
  var ww=0,wt=0;
  m.forEach(function(c){var w=Math.pow(0.5,(now-c.ts)/HL);wt+=w;if(c.curWin)ww+=w;});
  var wr=wt>0?Math.round(ww/wt*100):50;
  return{score:wr,count:m.length,msg:prevTech+(prevWin?'승':'패')+'후 '+wr+'%('+m.length+'건)'};
}

// ── 9. VWAP 밴드 + 거래량 프로파일 진입 구간 ──
function calcVWAPBands(cs){
  if(!cs||cs.length<10)return null;
  var cumVP=0,cumV=0,vwaps=[],typicals=[];
  for(var i=0;i<cs.length;i++){
    var tp=(cs[i].h+cs[i].l+cs[i].c)/3;cumVP+=tp*cs[i].v;cumV+=cs[i].v;
    vwaps.push(cumV>0?cumVP/cumV:tp);typicals.push(tp);
  }
  var cumSq=0;for(var j=0;j<cs.length;j++){var d=typicals[j]-vwaps[j];cumSq+=d*d*cs[j].v;}
  var std=cumV>0?Math.sqrt(cumSq/cumV):0;
  var lv=vwaps[vwaps.length-1];
  return{vwap:lv,upper1:lv+std,lower1:lv-std,upper2:lv+std*2,lower2:lv-std*2,std:std};
}
function volumeZoneScore(cs){
  var bands=calcVWAPBands(cs);
  if(!bands)return{bonus:0,zone:''};
  var price=cs[cs.length-1].c;
  if(price<=bands.lower1&&price>=bands.lower2)return{bonus:2,zone:'VWAP하단밴드'};
  if(Math.abs(price-bands.vwap)/bands.vwap<0.003)return{bonus:1,zone:'VWAP지지'};
  if(price>=bands.upper2)return{bonus:-2,zone:'VWAP과열'};
  if(price<=bands.lower2)return{bonus:-1,zone:'VWAP투매'};
  return{bonus:0,zone:''};
}

// ── 10. 섹터 상관관계 필터 (같은 섹터 집중 방지 + 모멘텀 감쇠) ──
var _sectorMomentum={};
try{var _sm=localStorage.getItem('htsSectorMomentum');if(_sm)_sectorMomentum=JSON.parse(_sm);}catch(_e){}
function updateSectorMomentum(){
  var sells=(mock.trades||[]).filter(function(t){return t.side==='sell';});
  var byDateSec={};
  sells.forEach(function(t){
    var stk=STOCKS.find(function(s){return s.tk===t.tk;});if(!stk)return;
    var sec=stk.sec||'기타';var key=t.date+'_'+sec;
    if(!byDateSec[key])byDateSec[key]={sec:sec,w:0,l:0,pnl:0};
    if((t.pnl||0)>0)byDateSec[key].w++;else byDateSec[key].l++;
    byDateSec[key].pnl+=(t.pnl||0);
  });
  var secDays={};
  Object.values(byDateSec).forEach(function(d){if(!secDays[d.sec])secDays[d.sec]=[];secDays[d.sec].push(d);});
  Object.keys(secDays).forEach(function(sec){
    var days=secDays[sec].slice(-5);
    var tw=days.reduce(function(s,d){return s+d.w;},0);
    var tt=days.reduce(function(s,d){return s+d.w+d.l;},0);
    var tp=days.reduce(function(s,d){return s+d.pnl;},0);
    var last2=days.slice(-2),prev3=days.slice(0,-2);
    var l2p=last2.reduce(function(s,d){return s+d.pnl;},0);
    var p3p=prev3.reduce(function(s,d){return s+d.pnl;},0);
    _sectorMomentum[sec]={wr:tt>0?Math.round(tw/tt*100):50,pnl:Math.round(tp),trades:tt,decaying:p3p>0&&l2p<p3p*0.3};
  });
  try{localStorage.setItem('htsSectorMomentum',JSON.stringify(_sectorMomentum));}catch(_e){}
}
function getCorrelationPenalty(tk){
  var stk=STOCKS.find(function(s){return s.tk===tk;});if(!stk)return{penalty:0,reason:''};
  var mySec=stk.sec||'기타';var penalty=0;var reasons=[];
  Object.keys(mock.positions||{}).forEach(function(heldTk){
    var pos=mock.positions[heldTk];if(!pos||pos.qty<=0)return;
    var hs=STOCKS.find(function(s){return s.tk===heldTk;});
    if(hs&&(hs.sec||'기타')===mySec){penalty-=2;reasons.push(mySec+'보유중');}
  });
  var sm=_sectorMomentum[mySec];
  if(sm&&sm.decaying){penalty-=1;reasons.push(mySec+'모멘텀감쇠');}
  if(sm&&sm.wr<35&&sm.trades>=5){penalty-=1;reasons.push(mySec+'승률'+sm.wr+'%');}
  return{penalty:penalty,reason:reasons.join('+')};
}

// ═══════════════════════════════════════════════
// 추가진화: 유전 알고리즘 + AI 하이브리드 기법 발굴
// ═══════════════════════════════════════════════
...
function _randomGene(retries){
  ...
  var g={
    ...
    volMin:1+(Math.random()*2.5),
    rsiLow:25+Math.floor(Math.random()*25),
    rsiHigh:55+Math.floor(Math.random()*20),
    bodyRatio:0.3+Math.random()*0.4,
    candleBull:Math.random()<0.7,
    lowerWickMin:Math.random()<0.4?1.5:0,
    useVwap:Math.random()<0.5,
    useRisingLows:Math.random()<0.4,
    usePrevBull:Math.random()<0.5,
    useDivergence:Math.random()<0.3,
    stopPct:2+Math.random()*2,
...
function _crossover(a,b){
  ...
  keys.forEach(function(k){child[k]=Math.random()<0.5?a[k]:b[k];});
  child.id='evo_'+Date.now()+'_'+Math.random().toString(36).slice(2,6);
  child.name='';child.generation=_evoPlus.generation+1;
  child.fitness=0;child.wins=0;child.losses=0;child.totalPnl=0;child.tested=0;
  return child;
}
function _mutate(gene){
  var g=JSON.parse(JSON.stringify(gene));
  var r=Math.random();
  if(r<0.12){g.maAlign=['none','short','triple'][Math.floor(Math.random()*3)];}
  else if(r<0.24){g.volMin=Math.max(0.8,g.volMin+(Math.random()-0.5)*1);}
  else if(r<0.36){g.rsiLow=Math.max(15,Math.min(55,g.rsiLow+Math.floor((Math.random()-0.5)*10)));g.rsiHigh=Math.max(g.rsiLow+10,Math.min(80,g.rsiHigh+Math.floor((Math.random()-0.5)*10)));}
  else if(r<0.46){g.bodyRatio=Math.max(0.1,Math.min(0.8,g.bodyRatio+(Math.random()-0.5)*0.2));}
  else if(r<0.54){g.candleBull=!g.candleBull;}
  else if(r<0.62){g.useVwap=!g.useVwap;g.useRisingLows=!g.useRisingLows;}
  else if(r<0.70){g.lowerWickMin=Math.random()<0.5?0:Math.max(0,Math.min(3,g.lowerWickMin+(Math.random()-0.5)*1));}
  else if(r<0.78){g.usePrevBull=!g.usePrevBull;g.useDivergence=!g.useDivergence;}
  else if(r<0.89){g.stopPct=Math.max(1.5,Math.min(5,g.stopPct+(Math.random()-0.5)*1));}
  else{g.targetMult=Math.max(1,Math.min(4,g.targetMult+(Math.random()-0.5)*0.5));}
  g.id='evo_'+Date.now()+'_'+Math.random().toString(36).slice(2,6);
  g.name='';g.generation=_evoPlus.generation+1;
  g.fitness=0;g.wins=0;g.losses=0;g.totalPnl=0;g.tested=0;
  return g;
}
function _testGeneOnCandles(gene,cs){
  ...
  var rsiArr=calcRSI(cls,14);
  var vwapArr=cs.length>=10?calcVWAP(cs):[];
  var results={wins:0,losses:0,pnl:0};
  for(var i=20;i<cs.length-5;i++){
    var lc=cs[i],pc=cs[i-1],pc2=cs[i-2];
    var lma5=ma5[i]||0,lma20=ma20[i]||0,lma60=ma60[i]||0;
    var rsi=rsiArr[i]||50,prevRsi=rsiArr[i-1]||50;
    var avg5v=0;for(var vi=i-5;vi<i;vi++)avg5v+=vls[vi]||0;avg5v/=5;
    var volR=avg5v>0?vls[i]/avg5v:1;
    var body=Math.abs(lc.c-lc.o),range=lc.h-lc.l||1;
    var lowerW=Math.max(0,Math.min(lc.c,lc.o)-lc.l);
    if(gene.maAlign==='triple'&&!(lma5>lma20&&lma20>lma60))continue;
    if(gene.maAlign==='short'&&!(lma5>lma20))continue;
    if(volR<gene.volMin)continue;
    if(rsi<gene.rsiLow||rsi>gene.rsiHigh)continue;
    if(gene.candleBull&&lc.c<=lc.o)continue;
    if(body/range<gene.bodyRatio)continue;
    if(gene.lowerWickMin>0&&lowerW<body*gene.lowerWickMin)continue;
    if(gene.useVwap&&vwapArr.length>i){if(lc.c<vwapArr[i]*0.998)continue;}
    if(gene.useRisingLows&&i>=2){if(!(cs[i-2].l<cs[i-1].l&&cs[i-1].l<lc.l))continue;}
    if(gene.usePrevBull&&pc.c<=pc.o)continue;
    if(gene.useDivergence){if(!(cls[i]<cls[i-1]&&rsi>prevRsi+2))continue;}
    var entryPr=lc.c;var stopPr=entryPr*(1-gene.stopPct/100);var targetPr=entryPr+((entryPr-stopPr)*gene.targetMult);
    var hit=false,pnl=0;
    for(var j=i+1;j<Math.min(i+10,cs.length);j++){
      if(cs[j].l<=stopPr){pnl=stopPr-entryPr;hit=true;break;}
      if(cs[j].h>=targetPr){pnl=targetPr-entryPr;hit=true;break;}
    }
    if(!hit)pnl=cs[Math.min(i+9,cs.length-1)].c-entryPr;
    if(pnl>0)results.wins++;else results.losses++;
    results.pnl+=pnl;
  }
  return results;
}
function _evalPool(){
  ...
  _evoPlus.pool.forEach(function(gene){
    ...
    allTks.forEach(function(tk){
      evalDays.forEach(function(day){
        var cs=null;
        try{cs=_peekSimCandlesFor(tk,day,390);}catch(_e){}
        if(!cs||cs.length<25)return;
        var r=_testGeneOnCandles(gene,cs);
        if(!r)return;
        totalW+=r.wins;totalL+=r.losses;totalPnl+=r.pnl;
      });
    });
    gene.wins=totalW;gene.losses=totalL;gene.totalPnl=totalPnl;gene.tested=1;
    var total=totalW+totalL;
    gene.fitness=total>=3?((totalW/total)*100*0.6+(totalPnl>0?Math.min(40,totalPnl/100):Math.max(-20,totalPnl/100))):0;
  });
}
function _evolveGeneration(){
  _evoPlus.generation++;
  _evalPool();
  _evoPlus.pool.sort(function(a,b){return b.fitness-a.fitness;});
  var top=_evoPlus.pool.slice(0,6);
  if(top.length&&top[0].fitness>_evoPlus.bestFitness)_evoPlus.bestFitness=top[0].fitness;
  var newPool=top.slice();
  for(var i=0;i<top.length-1&&newPool.length<12;i++){
    newPool.push(_crossover(top[i],top[i+1]));
  }
  while(newPool.length<16){
    if(top.length&&Math.random()<0.6){newPool.push(_mutate(top[Math.floor(Math.random()*top.length)]));}
    else{newPool.push(_randomGene());}
  }
  _evoPlus.pool=newPool;
  top.forEach(function(g){
    if(g.fitness>=40&&!_evoPlus.hall.find(function(h){return h.id===g.id;})){
      _evoPlus.hall.push(JSON.parse(JSON.stringify(g)));
      if(_evoPlus.hall.length>10)_evoPlus.hall.shift();
    }
  });
  _saveEvoPlus();
  return top[0]||null;
}
function _geneToDesc(g){
  var parts=[];
  if(g.maAlign==='triple')parts.push('3중정배열');
  else if(g.maAlign==='short')parts.push('단기정배열');
  parts.push('거래량'+g.volMin.toFixed(1)+'배+');
  parts.push('RSI'+g.rsiLow+'~'+g.rsiHigh);
  if(g.candleBull)parts.push('양봉');
  if(g.lowerWickMin>0)parts.push('아래꼬리');
  if(g.useVwap)parts.push('VWAP지지');
  if(g.useRisingLows)parts.push('저점상승');
  if(g.usePrevBull)parts.push('전봉양봉');
...
function registerEvolvedTechnique(gene){
  ...
  var name=baseName;
  var suffix=2;
  while(TECHNIQUES[name]){name=baseName+'_'+suffix;suffix++;}
...
async function _aiRefineGenes(){
  ...
  var sells=(mock.trades||[]).filter(function(t){return t.side==='sell';});
  var winTrades=sells.filter(function(t){return(t.pnl||0)>0;}).slice(-10);
  var lossTrades=sells.filter(function(t){return(t.pnl||0)<=0;}).slice(-10);
  var tradeData='';
  winTrades.forEach(function(t){tradeData+='[수익] '+t.nm+' '+(t.technique||'?')+' 손익+'+(t.pnl||0)+'\n';});
  lossTrades.forEach(function(t){tradeData+='[손실] '+t.nm+' '+(t.technique||'?')+' 손익'+(t.pnl||0)+'\n';});
  var topDesc=top.map(function(g,i){
  ...
  try{
    var data=await callClaude({model:"claude-sonnet-4-5",max_tokens:600,messages:[{role:"user",content:prompt}]},"진화+ AI정제");
    var text=(data.content&&data.content[0]&&data.content[0].text)||'';
    var jsonMatch=text.match(/\[\s*\{[\s\S]*?\}\s*\]/);
    if(jsonMatch){
      var suggestions;try{suggestions=JSON.parse(jsonMatch[0]);}catch(_jp){var m2=text.match(/\{[\s\S]*?"name"[\s\S]*?\}/g);if(m2)suggestions=m2.map(function(s){try{return JSON.parse(s);}catch(_e){return null;}}).filter(Boolean);}
      if(!suggestions||!suggestions.length){addMsg('ai','⚠ AI 응답 파싱 실패');return;}
      suggestions.forEach(function(s){
        var gene=_randomGene();
        ['maAlign','volMin','rsiLow','rsiHigh','bodyRatio','candleBull','lowerWickMin','useVwap','useRisingLows','usePrevBull','useDivergence','stopPct','targetMult'].forEach(function(k){if(s[k]!==undefined)gene[k]=s[k];});
        gene.name=s.name||'AI기법_G'+_evoPlus.generation;
        gene.generation=_evoPlus.generation;
        _evoPlus.pool.push(gene);
        _evoPlus.log.push({time:Date.now(),type:'ai',msg:'AI 제안: '+gene.name+' — '+(s.reason||_geneToDesc(gene))});
      });
      addMsg('ai','🤖 AI가 '+suggestions.length+'개 전략을 제안했습니다. 다음 세대에서 검증합니다.');
    }
...
var _evolveLoop=0;
var _evolveCyclesPerLoop=20;
var _evolveLoopCycleIdx=0;
var _evolveLoopHistory=[];
function _randomTradingDays(count){
  ...
  while(d<=today){
    var w=d.getDay();
    if(w!==0&&w!==6)allDays.push(d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0'));
    d.setDate(d.getDate()+1);
  }
  if(allDays.length<=count)return allDays;
  var picked=[];var used={};
  while(picked.length<count){
    var idx=Math.floor(Math.random()*allDays.length);
    if(!used[idx]){used[idx]=true;picked.push(allDays[idx]);}
  }
  return picked.sort();
}
function _takeSnapshot(){
  ...
  var pnl=sells.reduce(function(s,t){return s+(t.pnl||0);},0);
  var wr=total>0?Math.round(wins.length/total*100):0;
  var avgW=wins.length?wins.reduce(function(a,t){return a+(t.pnl||0);},0)/wins.length:0;
  var avgL=losses.length?losses.reduce(function(a,t){return a+Math.abs(t.pnl||0);},0)/losses.length:1;
  var rr=avgL>0?avgW/avgL:0;
  var totalLoss=losses.reduce(function(a,t){return a+Math.abs(t.pnl||0);},0);
  var pf=totalLoss>0?(wins.reduce(function(a,t){return a+(t.pnl||0);},0)/totalLoss):0;
...
function _startLearnPhase(){
  ...
  _evolveLoopCycleIdx++;
  // 20사이클 완료 → 루프 정리
  if(_evolveLoopCycleIdx>_evolveCyclesPerLoop){
    _finishLoopRound();
    return;
  }
  _evolvePhase='learn';
  ...
  if(cyEl)cyEl.textContent='루프 '+_evolveLoop+' ('+_evolveLoopCycleIdx+'/'+_evolveCyclesPerLoop+')';
  _evolveNextDay();
}
function _finishLoopRound(){
  if(!_evolveRunning)return;
  _evolvePhase='consolidate';
  _updBtn('🧬 L'+_evolveLoop+' 정리중');
  // 성과 프로파일 갱신
  ...
  var loopResult={
    loop:_evolveLoop,cycles:_evolveLoopCycleIdx-1,
    score:snap.score,wr:snap.wr,pnl:snap.pnl,trades:snap.trades,
    gaGen:_evoPlus.generation,
    evolved:Object.keys(TECHNIQUES).filter(function(k){return TECHNIQUES[k].evolved;}).length,
    patternDB:_patternDB.length,
    ts:Date.now()
  };
  if(_evolveLoopHistory.length>0){
    var prev=_evolveLoopHistory[_evolveLoopHistory.length-1];
    loopResult.deltaScore=snap.score-prev.score;
    loopResult.deltaPnl=snap.pnl-prev.pnl;
  }
  _evolveLoopHistory.push(loopResult);
  _evoPlus.log.push({time:Date.now(),type:'loop',msg:'루프'+_evolveLoop+' 완료 — 점수'+snap.score+' GA'+_evoPlus.generation+'세대 패턴'+_patternDB.length+'건'});
  _saveEvoPlus();
  addMsg('ai','🧬 루프 '+_evolveLoop+' 정리 완료\n• 종합점수: '+snap.score+'/100\n• GA: '+_evoPlus.generation+'세대\n• 패턴DB: '+_patternDB.length+'건\n• 등록기법: '+loopResult.evolved+'개\n'+(loopResult.deltaScore!==undefined?'• 점수변화: '+(loopResult.deltaScore>=0?'+':'')+loopResult.deltaScore:''));
  // 다음 루프 시작
  _evolveLoop++;
  _evolveLoopCycleIdx=0;
  setTimeout(function(){if(_evolveRunning)_startLearnPhase();},500);
}
...
function _onLearnDone(){
  ...
  computePerformanceProfile();
  _evolvePhase='ga';
  _updBtn('🧬 L'+_evolveLoop+' '+_evolveLoopCycleIdx+'/'+_evolveCyclesPerLoop+' GA');
  if(_evoPlus.pool.length<8){while(_evoPlus.pool.length<16)_evoPlus.pool.push(_randomGene());}
  _gaRound(0);
}
async function _gaRound(gen){
  if(!_evolveRunning)return;
  _updBtn('🧬 L'+_evolveLoop+' '+_evolveLoopCycleIdx+'/'+_evolveCyclesPerLoop+' GA'+(gen+1));
...
async function stopEvolvePlus(){
  ...
  var blacklisted=_geneBlacklist.length;
  var panel=document.getElementById('evolvePanel');
  if(panel){panel.style.display='';renderUnifiedPanel();}
  var ds=_evolveSnapshot.deltaScore;
  var loopSummary=_evolveLoopHistory.length?_evolveLoopHistory.map(function(h){return'L'+h.loop+'('+h.score+'점'+(h.deltaScore!==undefined?(h.deltaScore>=0?'+':'')+h.deltaScore:'')+')'}).join(' → '):'';
  // 진화 전용 결과 메시지
  var resultMsg='🧬 진화+ 결과\n═══════════════\n';
  resultMsg+='📊 '+_evolveLoop+'루프 '+_evolveCycle+'사이클 완료\n\n';
  resultMsg+='【학습 성과】\n';
  resultMsg+='• 종합점수: '+_evolveSnapshot.score+' → '+after.score+' ('+(ds>=0?'+':'')+ds+')\n';
  resultMsg+='• 매매: '+_evolveSnapshot.deltaTrades+'건 / 손익: '+(_evolveSnapshot.deltaPnl>=0?'+':'')+_evolveSnapshot.deltaPnl.toLocaleString()+'원\n\n';
  resultMsg+='【진화 데이터】\n';
  resultMsg+='• GA: '+_evoPlus.generation+'세대\n';
  resultMsg+='• 등록기법: '+evolved+'개 / 블랙리스트: '+blacklisted+'개\n';
  resultMsg+='• 패턴DB: '+_patternDB.length+'건\n';
  resultMsg+='• 원칙준수: '+getAdherenceInsight().msg+'\n\n';
  resultMsg+='【적응형 파라미터】\n';
  resultMsg+='• 손절: '+_adaptiveCfg.stop+'% / 비중: '+_adaptiveCfg.pos+'% / R/R: '+_adaptiveCfg.rr+'\n';
  resultMsg+='• 최소진입: '+_adaptiveCfg.minScore+'점\n\n';
  if(_perfProfile.byTech){
    ...
    Object.keys(_perfProfile.byTech).sort(function(a,b){return(_perfProfile.byTech[b].score||0)-(_perfProfile.byTech[a].score||0);}).forEach(function(k){
      ...
      resultMsg+='• '+t.grade+' '+k+': '+t.score+'점 ('+t.count+'건 승률'+t.winRate+'%)\n';
    });
  }
  if(loopSummary)resultMsg+='\n【루프 이력】\n'+loopSummary;
  addMsg('ai',resultMsg);
  showAlert('🧬 진화+ 결과',resultMsg);
}
function renderUnifiedPanel(){
  ...
  if(adEl){
    var ac=_adaptiveCfg;
    var sc=_perfProfile.recentScore||0;
    var scCol=sc>=70?'var(--g)':sc>=50?'var(--b)':sc>=30?'var(--a)':'var(--r)';
    var wr=_perfProfile.recentWr||'-';
    var rrVal=_perfProfile.recentRr||'-';
    var pfVal=_perfProfile.recentPf||'-';
    adEl.innerHTML=
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;"><span style="font-weight:700;">종합 점수</span><b style="font-size:14px;color:'+scCol+';">'+sc+'<span style="font-size:9px;color:var(--tm);">/100</span></b></div>'+
...
function _drawChartInner(){
  ...
  if(canvas._cx!=null){
    ...
    if(idx>=0&&idx<cs.length){
      const c=cs[idx];
      const ci=document.getElementById('cinfo');
      if(ci)ci.textContent=`O${fW(c.o)} H${fW(c.h)} L${fW(c.l)} C${fW(c.c)} V${fW(c.v)}`;
    }
  }
  ...
  try{
    ...
    _trades.forEach(function(t){
      ...
      var cx=cs[matchIdx];if(!cx)return;
      var yPr=toY(isBuy?cx.l:cx.h);
      ...
      if(isBuy){
        var y0=yPr+4;
        ctx.moveTo(x,y0+12);ctx.lineTo(x-6,y0);ctx.lineTo(x+6,y0);
      }else{
        var y0=yPr-4;
        ctx.moveTo(x,y0-12);ctx.lineTo(x-6,y0);ctx.lineTo(x+6,y0);
      }
...
(function _seedLearningOnce(){
  if(learningMemory && learningMemory.length>0) return;
  const _seed=[
    {date:'2026-04-01',category:'복기',text:'대형주 역추세 진입 금지: 삼성SDI·POSCO·LG화학 등 하락 중 진입 후 -3~4% 손실. 반등 기대한 물타기성 진입은 반복적 손실 원인.'},
    {date:'2026-04-01',category:'복기',text:'손절 기준 미준수: 전부 -3% 초과. 최대 -4.4%(LG화학). 설정한 -3% 룰 완전 무시 — 손절선 설정 후 반드시 시장가 즉시 실행.'},
    {date:'2026-04-01',category:'복기',text:'현재 강점 없음 — 승률 27%, 손실>수익, 손절 불이행. 뇌동매매 상태. 기본기부터 재정립 필요.'},
    {date:'2026-04-01',category:'복기',text:'수익 패턴 추출 불가 — 최근 5건 모두 손실. 수익 거래 내역 없어 패턴 분석 불가. 수익 거래가 쌓일 때까지 소량 매매.'},
    {date:'2026-04-01',category:'반성',text:'하락 추세 종목에 반등 기대하고 진입하는 습관. 추세 확인 없이 저점매수 시도 = 물타기. 반드시 5MA·20MA 정배열 확인 후 진입.'},
    {date:'2026-04-01',category:'반성',text:'손절선 -3% 설정해놓고 -4% 넘도록 보유. 손절은 판단이 아니라 기계적 실행. 진입 즉시 예약주문 필수.'},
    {date:'2026-04-01',category:'개선',text:'진입 전 체크: (1)추세 정배열 여부 (2)거래량 증가 (3)손절선 명확 (4)R/R 1.5 이상. 4개 중 1개라도 미달이면 진입 금지.'},
    {date:'2026-04-01',category:'개선',text:'1회 매매 비중 자산의 10% 이하로 축소. 확신 있어도 20% 초과 금지. 연속 손절 시 비중 더 줄일 것.'},
    {date:'2026-04-01',category:'멘토',text:'지금은 수익보다 규칙 준수가 목표. 손절 3번 연속 지키면 그게 성공. 수익은 규칙을 지키면 자연히 따라온다.'},
    {date:'2026-04-01',category:'멘토',text:'역추세 매매(하락 중 매수)는 숙련자도 실패한다. 초보는 반드시 추세 방향으로만 매매. 떨어지는 칼날 잡지 마라.'},
    {date:'2026-04-01',category:'심리',text:'FOMO(놓치면 안 될 것 같은 불안) 감지 시 즉시 매매 중단. 기회는 매일 온다. 오늘 못 사면 내일 산다.'},
    {date:'2026-04-01',category:'심리',text:'연속 손실 후 복수매매 충동 주의. 잃은 돈 빨리 벌려는 심리 = 더 큰 손실. 연속 3회 손절 시 당일 매매 종료.'},
  ];
  learningMemory=_seed;
  window.learningMemory=_seed;
  learnedDates=['2026-04-01'];
  window.learnedDates=learnedDates;
  localStorage.setItem('htsLearningMemory',JSON.stringify(_seed));
  localStorage.setItem('htsLearnedDates',JSON.stringify(learnedDates));
  try{
    saveToServer('htsLearningMemory',JSON.stringify(_seed));
    saveToServer('htsLearnedDates',JSON.stringify(learnedDates));
  }catch(e){}
  console.log('✅ 학습 노트 복구 완료: '+_seed.length+'개');
})();
function appendLesson(date, ai){
  ...
  // 완전 동일 텍스트 중복만 제거 (유사도 제거 삭제)
  newItems.forEach(ni => {
    const dupeIdx = learningMemory.findIndex(m =>
      m.category === ni.category && m.text.trim() === ni.text.trim()
    );
    if(dupeIdx >= 0) learningMemory.splice(dupeIdx, 1);
  });
  learningMemory.push(...newItems);
  if(learningMemory.length > 200) learningMemory = learningMemory.slice(-200);
...
function _addLearningReply(stockNm, pnl, tk){
  ...
  const pnlStr=pnl>=0?'+'+fW(pnl):fW(pnl);
  const icon=pnl>=0?'✅':'❌';
...
function _checkLearningWinRate(item){
  if(!item.replies || item.replies.length < 10) return;
  var wins=item.replies.filter(function(r){return (r.pnl||0)>0;}).length;
  var wr=Math.round(wins/item.replies.length*100);
  if(wr < 50 && item.category !== '실패교훈'){
    item._prevCategory = item.category;
    item.category = '실패교훈';
    item._winRate = wr;
    item.text = item.text + ' [성공률 '+wr+'% → 재검토 필요]';
    addDecisionLog('⚠ 실패 교훈 감지', item.text.slice(0,40)+'... ('+wr+'%)', '학습');
  }
}
...
function getLearningContext(maxItems){
  ...
  if(byCat['실패교훈'] && byCat['실패교훈'].length){
    out += '\n⚠ 아래는 10회+ 적용했으나 성공률 50% 미만인 교훈 — 이 기준은 의심하고 반대로 생각해볼 것:\n';
    byCat['실패교훈'].forEach(t=>{ out += `• [실패교훈] ${t}\n`; });
  }
    ...
    <div style="padding:14px;max-width:520px;">
      ...
      <div style="max-height:50vh;overflow-y:auto;display:flex;flex-direction:column;gap:6px;">
        ${learningMemory.length ? learningMemory.slice().reverse().map(l=>{
          const catCol=l.category==='실패교훈'?'#888':l.category==='반성'?'var(--r)':l.category==='개선'?'var(--g)':l.category==='멘토'?'var(--b)':'var(--p)';
...
async function stopBacktest(){
  ...
  _syncPlayBtn();
  renderBacktestPanel();
  // 백그라운드 일지 작성 완료 대기 — 진화+ 중에는 스킵 (일지 안 만듦)
  if(!_evolveRunning && backtest.pendingJournals && backtest.pendingJournals.length){
...
async function _backtestLoadDay(dateStr){
  ...
  // ★ 매일 강세섹터 새로 가져오기 — 진화+ 중에는 초기 분석 데이터 재사용 (비용 절감)
  if(typeof refreshHotSectors==='function' && !_evolveRunning){
...
async function _backtestEndOfDayInner(dt){
  ...
  backtest.dayIdx++;
  addDecisionLog(`📅 ${dt} 마감`, `매매 ${todayTrades.length}건 | 손익 ${dayPnl>=0?'+':''}${fW(dayPnl)}원 | 승 ${wins}·패 ${losses}`, '백테스트');
  renderBacktestPanel();
  // ★ 매매일지: 매매 유무에 상관없이 반드시 생성
  try{
    if(todayTrades.length > 0){
      // 매매가 있는 날: 즉시 기본 일지 저장 (AI 실패해도 내용 보장)
      const _sells = todayTrades.filter(t=>t.side==='sell');
      const _buyNames = [...new Set(todayTrades.filter(t=>t.side==='buy').map(t=>t.nm))].join(', ');
      const _sellSummary = _sells.map(t=>t.nm+' '+(t.pnl>=0?'+':'')+Math.round(t.pnl||0).toLocaleString()+'원').join(' / ');
      const _wr = _sells.length>0 ? Math.round(wins/_sells.length*100) : 0;
      const _rg = dayPnl>0?'수익':dayPnl<0?'손실':'무변동';
      const _baseJ = {
        summary: dt+' '+_rg+' | '+todayTrades.length+'건 '+(dayPnl>=0?'+':'')+fW(dayPnl)+'원 (승률 '+_wr+'%)',
        result_grade: _rg,
        why_bought: _buyNames ? '매수: '+_buyNames : '-',
        why_sold: _sellSummary || '-',
        good: dayPnl>0 ? '수익 실현' : wins>0 ? '일부 수익' : '매매 시도',
        bad: dayPnl<0 ? '손실 '+Math.round(dayPnl).toLocaleString()+'원' : '-',
        psychology: losses>=3 ? '연속 손절 주의' : '정상',
        improvement: dayPnl<0 ? '손절 기준 점검' : '전략 유지',
        aiGenerated: false,
        trades: todayTrades.map(t=>(t.side==='buy'?'매수':'매도')+' '+t.nm+' '+t.qty+'주 @'+fW(t.price)+(t.pnl?(' ('+(t.pnl>=0?'+':'')+Math.round(t.pnl).toLocaleString()+'원)'):'')).join('\n'),
      };
      saveJEntry(dt, _baseJ, dayPnl, wins, _sells.length, _baseJ.trades);
      addDecisionLog(`📓 ${dt} 일지 저장`, todayTrades.length+'건 기록', '학습');
      // AI 평가도 시도 (실패해도 위 기본 일지가 남음) — 진화+ 중에는 스킵 (비용 절감)
      if(typeof autoSaveJournalOnTrade === 'function' && !_evolveRunning){
        const _p = autoSaveJournalOnTrade(dt).catch(e=>console.warn('AI일지:', e.message));
      ...
      const noTradeEntry = {
        ...
        psychology: '인내심 유지', improvement: '시그널 재점검',
        phase_check: '관망',
        mentor_comment: '기다림도 매매다',
        aiGenerated: false, trades: '(매매 없음)',
      };
      saveJEntry(dt, noTradeEntry, 0, 0, 0, '');
    }
...
function _backtestReport(){
  ...
  let msg=`📊 백테스트 종료\n\n` +
    ...
    `• 총 매매: ${totalTrades}건\n` +
    `• 총 손익: ${totalPnl>=0?'+':''}${fW(totalPnl)}원 (${pnlPct}%)\n` +
    `• 승률: ${winRate}% (${totalWins}승 ${totalLosses}패)\n` +
    (best ? `• 최고: ${best.date} (+${fW(best.pnl)})\n` : '') +
    (worst ? `• 최악: ${worst.date} (${fW(worst.pnl)})\n` : '');
  // 진화 엔진: 백테스트 후 성과 프로파일 갱신
  try{
    var pp=computePerformanceProfile();
    if(pp.byTech){
      var evolveMsg='\n🧬 진화 엔진 업데이트:\n';
      Object.keys(pp.byTech).forEach(function(k){
        var t=pp.byTech[k];if(t.count<2)return;
        evolveMsg+=`  ${k}: ${t.grade}등급 ${t.score}점 승률${t.winRate}% R/R${t.rr}\n`;
      });
      evolveMsg+=`  적응형→ 손절${_adaptiveCfg.stop}% 비중${_adaptiveCfg.pos}% 최소R/R${_adaptiveCfg.rr}`;
      msg+=evolveMsg;
    }
  }catch(_e){console.warn('perfProfile:',_e.message);}
  if(!_evolveRunning){
    addMsg('ai', msg);
  ...
  el.innerHTML = '<span style="color:var(--tm);font-weight:700;margin-right:6px;">⚡ 실시간 매매</span>' +
    recent.map(function(t){
      ...
      return `<span style="display:inline-block;margin-right:10px;padding:1px 4px;border-radius:3px;background:var(--pan);border:1px solid var(--br);">
        ...
        <span style="color:var(--ts);">${fW(t.price||t.pr)}원×${t.qty}</span>
        ${pnlTxt}
      </span>`;
    }).join('');
}
...
function renderBacktestPanel(){
  ...
  el.innerHTML = `
    <div style="display:flex;align-items:center;gap:8px;">
      ...
      <span style="font-family:var(--mono);font-size:10px;font-weight:700;color:${realtimePnl>=0?'var(--g)':'var(--r)'};">${realtimePnl>=0?'+':''}${fW(realtimePnl)}원</span>
      <button class="ibtn" onclick="backtestDiagnose()" style="font-size:9px;padding:2px 6px;background:var(--a);color:#fff;border:none;">🔍 검진</button>
...
function updPrice(c){
  ...
  if(siPr){siPr.textContent=fW(pr);siPr.className="si-pr "+cls;}
  ...
  const chPr=document.getElementById("chartHdrPr");if(chPr) chPr.textContent=fW(pr);
  ...
  if(obPr){obPr.textContent=fW(pr);obPr.className="ob-cur-p "+cls;}
...
function renderOB(){
  ...
  for(let i=10;i>=1;i--){
    ...
    sellRows.push(`<div class="ob-r" onclick="document.getElementById('ofPr').value=${pr};updOSum()"><div class="ob-bg s" style="width:${w}%"></div><div class="ob-p" style="color:#2563eb;">${fW(pr)}</div><div class="ob-q">${q.toLocaleString()}</div></div>`);
  }
  // 매수 10호가 (국내 표준: 매수=빨강)
  const buyRows=[];
  for(let i=1;i<=10;i++){
    ...
    totalBuy+=q;
    const w=Math.min(100,(q/maxQ*100)).toFixed(0);
    buyRows.push(`<div class="ob-r" onclick="document.getElementById('ofPr').value=${pr};updOSum()"><div class="ob-bg b" style="width:${w}%"></div><div class="ob-p" style="color:#dc2626;">${fW(pr)}</div><div class="ob-q">${q.toLocaleString()}</div></div>`);
  }
  // 총잔량 헤더 (매도)
...
function renderTH(){
  ...
  for(let i=0;i<25;i++){
    ...
    rows.push(`<div style="display:flex;align-items:center;padding:2px 8px;border-bottom:1px solid var(--br);gap:5px;">
      ...
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;flex:1;text-align:right;color:${col};">${arrow} ${fW(pr)}</span>
...
function onCredChange(){
  ...
  else{info.style.display="block";const avail=getCredAvail();info.textContent=credType==="credit"?`신용 가능: ${fW(avail)}원 (이자 연 ${cfg.crate}%)`:`미수 가능: ${fW(avail)}원 (이자 연 ${cfg.mrate}% | 미결제→반대매매)`;}
  updOSum();
}
...
function updOSum(){
  ...
  document.getElementById("ofSum").innerHTML=`예상: <b>${fW(tot)}</b>원 | 비용 ${Math.round(fee+tax).toLocaleString()}원${rr}${credType!=="cash"?` | <span style="color:var(--a)">${credType==="credit"?"신용":"미수"}</span>`:""}`;
  ...
  if(oSide==="sell"&&mock.positions[activeTk]&&pr&&qty){
    ...
    if(pnlEl)pnlEl.innerHTML+=` | 실질 <b style="color:${netPnl>=0?"var(--g)":"var(--r)"}">${netPnl>=0?"+":""}${fW(netPnl)}원</b>`;
  }
}
...
그래도 매수하시겠습니까?`))return false;
  ...
  checkBrainDong("buy",pr,qty,stk);
    ...
    // ── 학습 엔진 v2: 매도 시 패턴/원칙/시장상태 기록 ──
    try{
      var _sellTech=(_buyTrade||{}).technique||'';
      var _sellCond=captureEntryConditions();
      recordPattern('sell',_sellTech,_sellCond,pnl);
      var _regime2=detectMarketRegime();
      recordRegimePerf(_regime2,_sellTech,pnl);
      var _rc2=_tradeContext[activeTk];
      if(_rc2&&_rc2._ruleCheck)recordAdherence(_sellTech,_rc2._ruleCheck,pnl);
      if((mock.trades||[]).filter(function(t){return t.side==='sell';}).length%10===0){evolveFeatureWeights();updateSectorMomentum();}
      var _lastTrade=(mock.trades||[]).slice(-1)[0];
      if(_lastTrade)recordPatternChain(_lastTrade);
    }catch(_le){}
  ...
  if(oSide==='sell'&&(mock.trades||[]).filter(function(t){return t.side==='sell';}).length%5===0){try{computePerformanceProfile();}catch(_e){}}
...
function checkStopOrders(){
  ...
  Object.entries(stopOrders).forEach(([tk,so])=>{
    ...
    // 다중신호 청산 최적화
    try{
      var _exit=computeExitSignal(tk);
      if(_exit&&_exit.action==='EXIT_ALL'&&pos.qty>0){
        var svTk=activeTk,svSide=oSide,svType=oType,svCred=credType;
        activeTk=tk;oSide="sell";oType="market";credType="cash";
        document.getElementById("ofQty").value=pos.qty;
        window._pendingSellReason='다중청산신호: '+_exit.reasons.join('+');
        submitOrder(true);
        activeTk=svTk;oSide=svSide;oType=svType;credType=svCred;
        delete stopOrders[tk];
        addMsg("ai",'🔶 다중신호 청산! '+stk.nm+' (긴급도 '+_exit.urgency+')\n'+_exit.reasons.join(' · '));
        return;
      }
      if(_exit&&_exit.action==='TIGHTEN_STOP'&&_exit.newStop>so.stop){
        so.stop=_exit.newStop;
        addDecisionLog('['+stk.nm+'] 손절선 상향','다중신호: '+_exit.reasons.join('+')+' → '+fW(_exit.newStop),'리스크');
      }
    }catch(_ex){}
    // Trailing update
    ...
    if(so.t1done&&!so.t2done&&pr>=so.t2&&pos.qty>0){
      ...
      so.t2done=true;
      addMsg("ai",`🟢 2차 익절! ${stk.nm} ${fW(pr)}원 (30% 청산)\n잔여 20% 트레일링 스탑 유지\n[Phase 9-2]`);
      renderPort();
    }
  });
  // Phase 3-8: 가격 알림 체크
...
function checkPriceAlerts(){
  ...
  STOCKS.forEach(stk=>{
    priceAlerts.forEach((al,i)=>{
      ...
      if(hit){
        al.fired=true;
        showAlert(`📣 가격 알림: ${stk.nm}`,`${stk.nm}이(가) ${fW(al.price)}원 ${al.dir==="above"?"이상":"이하"}에 도달했습니다.
현재가: ${fW(stk.pr)}원
...
평균 수익: ${fW(avgWin)}원 | 평균 손실: ${fW(avgLoss)}원
총 손익: ${totalPnl>=0?'+':''}${fW(totalPnl)}원`;
  const winDetail=wins.length?wins.slice(-15).map(t=>`+${Math.round(t.pnl||0).toLocaleString()}원 ${t.nm} ${fW(t.price)}원 (${t.time||t.barTime||''}) ${t.auto?'AI':'수동'}`).join('\n'):'(수익 거래 없음)';
  const lossDetail=losses.length?losses.slice(-15).map(t=>`${Math.round(t.pnl||0).toLocaleString()}원 ${t.nm} ${fW(t.price)}원 (${t.time||t.barTime||''}) ${t.auto?'AI':'수동'}`).join('\n'):'(손실 거래 없음)';
  const byStock={};
  trades.forEach(t=>{
    ...
    if(!wins.length){wEl.innerHTML="<span style='color:var(--tm);'>수익 거래 없음</span>";}
      ...
      wEl.innerHTML=`<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px;">
        <div style="background:rgba(5,192,114,.06);border-radius:8px;padding:8px;text-align:center;">
          ...
          <div style="font-family:var(--mono);font-size:13px;font-weight:700;color:var(--g);">+${fW(avgWin)}원</div>
        </div>
        <div style="background:rgba(5,192,114,.06);border-radius:8px;padding:8px;text-align:center;">
          <div style="font-size:9px;color:var(--tm);">최대 수익</div>
          <div style="font-family:var(--mono);font-size:13px;font-weight:700;color:var(--g);">+${fW(maxWin)}원</div>
        </div>
      </div>
    ...
    if(!losses.length){lEl.innerHTML="<span style='color:var(--tm);'>손실 거래 없음</span>";}
      ...
      lEl.innerHTML=`<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px;">
        <div style="background:rgba(240,62,62,.06);border-radius:8px;padding:8px;text-align:center;">
          ...
          <div style="font-family:var(--mono);font-size:13px;font-weight:700;color:var(--r);">${fW(avgLoss)}원</div>
        </div>
        <div style="background:rgba(240,62,62,.06);border-radius:8px;padding:8px;text-align:center;">
          <div style="font-size:9px;color:var(--tm);">최대 손실</div>
          <div style="font-family:var(--mono);font-size:13px;font-weight:700;color:var(--r);">${fW(maxLoss)}원</div>
        </div>
      </div>`;
    }
  }
...
function checkPsychWarnings(){
  ...
  Object.entries(mock.positions).forEach(([tk,pos])=>{
    ...
    if(pnlPct>0&&pnlPct<1&&so&&stk.pr<so.t1){
      warnings.push(`처분효과 주의: ${stk.nm} +${pnlPct.toFixed(1)}% — 목표가(${fW(so.t1)}) 미달. 조기청산 충동 = 처분효과`);
    }
    ...
    ${trades.map(t=>{
      ...
      return`<div style="position:relative;padding:5px 0 5px 12px;margin-bottom:2px;">
        ...
        <div style="display:flex;align-items:center;gap:6px;">
          ...
          <span style="font-family:var(--mono);font-size:9px;color:var(--ts);">${fW(t.price)}원 × ${t.qty}주</span>
...
// ═══ 매매 상세 기록 — 날짜별 종목/진입사유/매도사유 ═══
function renderTradeDetailLog(){
  const el=document.getElementById('tradeDetailLog'); if(!el) return;
  const trades=mock.trades||[];
...
function renderLearningMgmt(){
  ...
  const cats={반성:'var(--r)',개선:'var(--b)',멘토:'var(--p)',심리:'var(--a)',복기:'var(--g)',실패교훈:'#888'};
...
function deleteLearningItem(idx){
  if(!learningMemory||idx<0||idx>=learningMemory.length) return;
  learningMemory.splice(idx,1);
  window.learningMemory=learningMemory;
  localStorage.setItem('htsLearningMemory',JSON.stringify(learningMemory));
  saveToServer('htsLearningMemory',JSON.stringify(learningMemory));
  renderLearningMgmt();
...
async function cleanupLearning(){
  ...
  _backupLearning();
  var _allBtns=document.querySelectorAll('[onclick*="cleanupLearning"]');
  _allBtns.forEach(function(b){b.textContent='🔄 정리하는 중...';b.disabled=true;b.style.opacity='0.6';});
  addMsg('ai','🔄 AI 학습 정리 시작... ('+learningMemory.length+'개 분석 중)');
  var _backup = JSON.parse(JSON.stringify(learningMemory));
  try{
    var prompt='너는 트레이딩 멘토. 아래 학습 노트에서 **중복된 내용만** 병합 정리해줘.\n\n'+
      '정리 원칙:\n'+
      '- 완전히 같은 뜻의 내용이 2개 이상이면 → 가장 구체적인 1개로 병합\n'+
      '- 중복이 아닌 항목은 원문 그대로 유지 (절대 삭제 금지)\n'+
      '- 마크다운 헤더(##, #)만 있는 줄 → 삭제 가능\n'+
      '- 내용이 있는 항목은 비슷해 보여도 유지 (각각 다른 상황의 교훈일 수 있음)\n'+
      '- 원본 개수의 70% 이상을 반드시 유지할 것\n\n'+
      '현재 노트 ('+learningMemory.length+'개):\n'+
      learningMemory.map(function(l,i){return '- ['+l.category+'] '+l.text;}).join('\n')+
      '\n\nJSON만: {"notes":[{"category":"반성|개선|멘토|심리","text":"정리된 내용"},...], "summary":"뭘 했는지 한줄"}';
    ...
    if(data.error) throw new Error('API: '+(typeof data.error==='string'?data.error:JSON.stringify(data.error)));
    var text=(data.content&&data.content[0]&&data.content[0].text)||'';
    if(!text) throw new Error('AI 응답이 비어있음');
    var result=robustJsonParse(text);
    if(!result.notes||!Array.isArray(result.notes)||result.notes.length<3){
      throw new Error('AI 결과 부족 ('+((result.notes||[]).length)+'개) — 응답: '+text.slice(0,100));
    }
    var before=learningMemory.length;
    var today=typeof todayStr==='function'?todayStr():new Date().toISOString().slice(0,10);
    learningMemory=result.notes.map(function(n){return{
      date:today, category:n.category||'개선', text:(n.text||'').trim()
    };}).filter(function(n){return n.text.length>=5;});
    if(learningMemory.length < Math.min(5, before)){
      learningMemory=_backup;
      window.learningMemory=_backup;
      addMsg('ai','⚠ AI 정리 결과가 너무 적음 → 복원됨');
      _allBtns.forEach(function(b){b.textContent='AI 정리';b.disabled=false;b.style.opacity='1';});
      return;
    }
    window.learningMemory=learningMemory;
    localStorage.setItem('htsLearningMemory',JSON.stringify(learningMemory));
    ...
    renderLearningMgmt();
    updateLearnerStage();
    addMsg('ai','✅ 학습 노트 AI 정리 완료\n'+before+'개 → '+learningMemory.length+'개\n'+(result.summary||''));
  }catch(e){
    learningMemory=_backup;
    window.learningMemory=_backup;
    addMsg('ai','⚠ 정리 실패 (복원됨): '+e.message);
  }
  _allBtns.forEach(function(b){b.textContent='AI 정리';b.disabled=false;b.style.opacity='1';});
}
async function rebuildLearningFromJournals(){
  ...
  entries.forEach(e=>{
    const d=e.date;
    if(e.bad && e.bad!=='-' && e.bad!=='없음' && e.bad.length>=5) rebuilt.push({date:d,category:'반성',text:e.bad});
    if(e.improvement && e.improvement!=='-' && e.improvement.length>=5) rebuilt.push({date:d,category:'개선',text:e.improvement});
    if(e.mentor_comment && e.mentor_comment!=='-' && e.mentor_comment.length>=5) rebuilt.push({date:d,category:'멘토',text:e.mentor_comment});
    if(e.psychology && e.psychology!=='-' && e.psychology!=='정상' && e.psychology.length>=5) rebuilt.push({date:d,category:'심리',text:e.psychology});
    if(e.mistakes && e.mistakes!=='-' && e.mistakes.length>=5) rebuilt.push({date:d,category:'반성',text:e.mistakes});
  });
  const unique=[];
  rebuilt.forEach(r=>{
    if(!unique.some(u=>u.date===r.date&&u.text===r.text)) unique.push(r);
  });
  learningMemory=unique.slice(-80);
  window.learningMemory=learningMemory;
  localStorage.setItem('htsLearningMemory',JSON.stringify(learningMemory));
  saveToServer('htsLearningMemory',JSON.stringify(learningMemory));
  updateLearnerStage();
  addMsg('ai','✅ 일지에서 학습 노트 복구 완료: '+learningMemory.length+'개 항목');
}
...
const REVIEW_PROMPTS = {
  1: (trades, pnl) => `아래 매매를 Phase 8~12 기준으로 분석해줘.\n\n매매: ${trades.slice(-1).map(t=>`${t.nm} ${t.side==='buy'?'매수':'매도'} ${fW(t.price)}원 ${t.qty}주 손익${t.pnl>=0?'+':''}${Math.round(t.pnl||0)}원`).join(', ')}\n\n평가:\n1. 진입 타점 적절성 (Phase 8)\n2. 청산 타이밍 (Phase 9)\n3. R/R 수준\n4. 원칙 준수 여부\n5. 반복 실수 여부\n6. 같은 상황 다시 오면?`,
...
function updateTrailPanel(){
  ...
  el.innerHTML=`<div style="color:${up?"var(--g)":"var(--r)"};font-weight:700;">${up?"+":""}${pnlPct}%</div>`+
    ...
    `<div style="color:var(--r);">손절 ${fW(so.stop)}</div>`+
    (so.t1?`<div style="color:var(--g);">1차 ${fW(so.t1)} ${so.t1done?"✅":""}</div>`:``)+ 
...
function setTrailPct(pct){
  ...
  updateTrailPanel();
  addDecisionLog(`트레일링 스탑 -${pct}% 설정`,`현재가 ${fW(stk.pr)} → 손절 ${fW(so.stop)}`,"Phase 9-3");
}
...
async function _runScreeningAsync(){
  ...
  for(var _ti=0;_ti<allTks.length;_ti++){
    ...
    // 11-B. 진화기법 매칭 — 등록된 진화기법 조건 체크
    try{
      var _evoTechs=Object.keys(TECHNIQUES).filter(function(k){return TECHNIQUES[k].evolved&&TECHNIQUES[k].gene;});
      _evoTechs.forEach(function(ek){
        var eg=TECHNIQUES[ek].gene;if(!eg)return;
        if(eg.maAlign==='triple'&&!(lma5>lma20&&lma20>lma60))return;
        if(eg.maAlign==='short'&&!(lma5>lma20))return;
        if(volR<eg.volMin)return;
        if(lrsi<eg.rsiLow||lrsi>eg.rsiHigh)return;
        if(eg.candleBull&&lc.c<=lc.o)return;
        if(body>0&&(body/(lc.h-lc.l||1))<eg.bodyRatio)return;
        score+=3;tags.push(ek);
      });
    }catch(_evo){}
    ...
    // 진화: 기법 성과 기반 점수 보정
    var _detTech=_isPullback?'눌림목':_isBreakout?'돌파':_isGap?'갭상승':_isFirstBar?'첫봉':_isMomentum?'이슈테마':'기타';
    var _tgCand=getTechGrade(_detTech);
    if(_tgCand.grade!=='?'){
      if(_tgCand.grade==='A'){score=Math.round(score*1.2);tags.push('A등급↑');}
      else if(_tgCand.grade==='D'){score=Math.round(score*0.85);tags.push('D등급↓');}
      var _techRR=parseFloat(_tgCand.rr)||0;
      if(_techRR>=2){score+=1;tags.push('R/R'+_techRR);}
      var _techPF=parseFloat(_tgCand.pf)||0;
      if(_techPF>=1.5){score+=1;tags.push('PF'+_techPF);}
    }
    // 학습 메모리: 성공 경험만 가산 (감점은 AI 프롬프트에서 처리)
    try{
      var _lmGood=(window.learningMemory||[]).slice(-15).filter(function(l){
        var txt=l.text||'';
        return(txt.indexOf(_detTech)!==-1||txt.indexOf(stk.nm)!==-1)&&(l.category==='성공'||l.category==='수익');
      });
      if(_lmGood.length>=1){score+=1;tags.push('학습성공');}
    }catch(_lme){}
    if((_perfProfile.recentScore||50)>=70){score+=1;tags.push('성과우수');}
    // ── 학습 엔진 v2 적용 ──
    // 패턴 DB 조회: 유사 조건의 과거 승률
    var _entryCond=captureEntryConditions();
    var _patQ=queryPatternDB(_entryCond,_detTech);
    if(_patQ.count>=5){
      if(_patQ.score>=65){score+=2;tags.push('패턴승률'+_patQ.score+'%');}
      else if(_patQ.score<=30){score-=1;tags.push('패턴경고'+_patQ.score+'%');}
    }
    // 강의 원칙 체크
    var _prevCount2=(_kisChartMeta&&_kisChartMeta.prevCount)||0;
    var _prev20H=Math.max.apply(null,cs.slice(-21,-1).map(function(c){return c.h;}));
    var _ruleCheck=checkLectureRules(_detTech,lc,lma5,lma20,volR,_prev20H,sim.idx,_prevCount2,lrsi,pc,pc2);
    if(_ruleCheck.failed>=2){score-=1;tags.push('원칙위반'+_ruleCheck.failed);}
    // 가중치 진화 적용
    score=applyFeatureWeights(score,_entryCond);
    // 시장 상태 적응
    var _regime=detectMarketRegime();
    var _regAdv=getRegimeAdvice(_regime,_detTech);
    if(_regAdv.adj!==1){score=Math.round(score*_regAdv.adj);if(_regAdv.msg)tags.push(_regAdv.msg);}
    // VWAP 밴드 진입 구간
    var _vz=volumeZoneScore(cs);
    if(_vz.bonus){score+=_vz.bonus;if(_vz.zone)tags.push(_vz.zone);}
    // 섹터 상관관계 필터
    var _cp=getCorrelationPenalty(tk);
    if(_cp.penalty){score+=_cp.penalty;if(_cp.reason)tags.push(_cp.reason);}
    // 순차 패턴 체인
    var _lastSell=(mock.trades||[]).filter(function(t){return t.tk===tk&&t.side==='sell';}).slice(-1)[0];
    if(_lastSell){var _chain=queryPatternChain(_lastSell.technique||'?',(_lastSell.pnl||0)>0?1:0);if(_chain.score>=65&&_chain.count>=5){score+=1;tags.push('체인승률'+_chain.score+'%');}else if(_chain.score<=25&&_chain.count>=5){score-=1;tags.push('체인경고');}}
    // 드로다운 서킷브레이커
    var _ec=updateEquityCurve();
    if(_ec.tierHit>=3){addDecisionLog('[서킷브레이커]','드로다운 3%+ — 전종목 매매 중지','리스크');scored=[];break;}
    if(_ec.tierHit>=2){score=Math.round(score*0.5);tags.push('DD경고Lv2');}
    scored.push({tk:tk,stk:stk,score:score,tags:tags,lc:lc,lma5:lma5,lrsi:lrsi,volR:volR,detTech:_detTech,entryCond:_entryCond,ruleCheck:_ruleCheck,regime:_regime});
  }

  if(!scored.length){
    var _noTradeReason = '전 '+allTks.length+'종목 게이트 미달 (보유중 제외 '+Object.keys(mock.positions||{}).filter(function(t){return mock.positions[t]&&mock.positions[t].qty>0;}).length+'종목)';
    if(dayLossRate>=cfg.dayloss*0.7) _noTradeReason += ' / 일손실 '+dayLossRate.toFixed(1)+'%';
  ...
  try{
    ...
    if(heldPos && best.tk !== heldPos[0]){
      ...
      if(best.score - heldScore >= 5 && heldPnlPct > -1.5){
        ...
        document.getElementById("ofQty").value=pos.qty;
        window._pendingSellReason='갈아타기 — '+best.stk.nm+'('+best.score+'점)으로 교체';
        submitOrder(true);
        activeTk=sv;oSide=svSide;oType=svType;credType=svCred;
  ...
  // 진화: 적응형 최소 점수 (성과 좋으면 진입 기준 낮춤, 나쁘면 높임)
  var _minEntry=Math.max(2,_adaptiveCfg.minScore||6);
  if(best.score<_minEntry){
    var _whyPass = best.tags.join(' ')||'조건미달';
    _whyPass += ' (최고 '+best.score+'점/'+_minEntry+'점 필요 — 적응형)';
  ...
  if(autoState.level>=3 && (sim&&sim.speed>=300)){
    ...
    // 진입 등급 (백테스트 모드 — 학습용이므로 적극적):
    // - score≥4: BUY
    // - score≥3 + 확정시그널 또는 패턴태그: BUY
    // - score≥2: 관찰
    const hasConfirm = best.tags.indexOf('확정시그널') !== -1;
    const hasPattern = best.tags.indexOf('눌림목')!==-1 || best.tags.indexOf('돌파')!==-1 || best.tags.indexOf('갭상승')!==-1 || best.tags.indexOf('첫봉')!==-1 || best.tags.indexOf('대장첫숨')!==-1;
    if(best.score >= 4 || (best.score >= 3 && (hasConfirm || hasPattern))){
      var lcQ=best.lc;
      ...
      if(best.entryCond)recordPattern('buy',best.detTech,best.entryCond,0);
      if(best.ruleCheck){if(!_tradeContext[best.tk])_tradeContext[best.tk]={};_tradeContext[best.tk]._ruleCheck=best.ruleCheck;}
  ...
  if(autoState.level>=3){
    // 진화+ 중에만 규칙 기반 (실매매에서는 항상 Claude 사용)
    if(_evolveRunning){
      var _hasConf2=best.tags.indexOf('확정시그널')!==-1;
      var _hasPat2=best.tags.indexOf('눌림목')!==-1||best.tags.indexOf('돌파')!==-1||best.tags.indexOf('갭상승')!==-1||best.tags.indexOf('첫봉')!==-1;
      if(best.score>=5||best.score>=4&&(_hasConf2||_hasPat2)){
        var lcQ2=best.lc;
        addDecisionLog('['+best.stk.nm+'] ✅ 진입 ('+best.score+'점·학습기반)', best.tags.join(' · '), '학습판단');
        var _bestTech2=best.tags.indexOf('대장첫숨')!==-1?'대장첫숨':'';
        if(best.entryCond)recordPattern('buy',best.detTech,best.entryCond,0);
        if(best.ruleCheck){if(!_tradeContext[best.tk])_tradeContext[best.tk]={};_tradeContext[best.tk]._ruleCheck=best.ruleCheck;}
        try{await execAutoBuy(lcQ2.c,best.stk,best.score,_bestTech2);}catch(_e){}
      }else{
        addDecisionLog('['+best.stk.nm+'] 관망 ('+best.score+'점)', best.tags.join(' '), '학습판단');
      }
      return;
    }
    // AI 판단 캐시: 같은 종목+비슷한 점수+5분 이내 → 재사용
    if(!window._aiDecisionCache)window._aiDecisionCache={};
    var _cacheKey=best.tk+'_'+Math.floor(best.score/2);
    var _cached=window._aiDecisionCache[_cacheKey];
    if(_cached&&Date.now()-_cached.ts<120000){
      if(_cached.decision==='BUY'&&_cached.confidence>=65){
        addDecisionLog('['+best.stk.nm+'] ✅ AI캐시매수','캐시 재사용 (신뢰도'+_cached.confidence+'%)','캐시');
        var _bestTech3=best.tags.indexOf('대장첫숨')!==-1?'대장첫숨':'';
        if(best.entryCond)recordPattern('buy',best.detTech,best.entryCond,0);
        try{await execAutoBuy(best.lc.c,best.stk,best.score,_bestTech3);}catch(_e){}
      }
      return;
    }
    try{
      ...
      var prompt='단타 트레이딩 멘토. 자동매매 최종 진입 여부.\n\n'+
        ...
        '대장첫숨: 이슈/재료 동반 필수 + 대장주 장대양봉 후 첫 음봉 → 비중 1/3.\n\n'+
        _lec +
        _learn +
        '【종목】 '+best.stk.nm+'('+best.tk+') '+best.stk.sec+'\n'+
        '【시간】 '+(sim.date||'')+' '+(best.lc.t||'')+'\n'+
        '【기술적】 가격 '+fW(lc2.c)+'원 | RSI '+best.lrsi+' | 거래량 평균x'+best.volR.toFixed(1)+'\n'+
        '【조건】 '+best.tags.join(' · ')+'\n'+
        (_sectorLine ? '【수급/섹터】 '+_sectorLine+'\n' : '')+
        (_shortLine ? '【공매도】 '+_shortLine+'\n' : '')+
        (_newsItems ? '【최근 공시】\n'+_newsItems+'\n' : '')+
        (_mkt && _mkt!=='데이터없음' ? '【시황】\n'+_mkt+'\n' : '')+
        '【리스크】 손절 '+fW(stopPr)+' | 목표 '+fW(t1Pr)+' | R/R 1:'+rr+'\n\n'+
      ...
      window._aiDecisionCache[_cacheKey]={decision:ai.decision||'PASS',confidence:ai.confidence||0,ts:Date.now()};
      if(ai.decision==='BUY'&&ai.confidence>=65){
        ...
        addMsg('ai','🤖 AI자동매수\n\n종목: '+best.stk.nm+' '+fW(lc2.c)+'원\n신뢰도: '+ai.confidence+'%\n\n✅ '+ai.reason+'\n'+_factorsTxt+'⚠ '+ai.risk+'\n\n손절: '+fW(stopPr)+' | 목표: '+fW(t1Pr)+' | R/R 1:'+rr);
    ...
    updAdvBoxes('📊 진입신호 — '+best.stk.nm,best.tags.join(' · ')+'\n진입: '+fW(lc3.c)+' 손절: '+fW(stopPr2)+' 목표: '+fW(t1Pr2)+' R/R:1:'+rr2+'\n⚡ 직접 주문창에서 실행');
...
function runAutoStep(cs){
  if(!autoState.running)return;
  // 백테스트 중 봉 단위 강세섹터 갱신 — 진화+ 중에는 스킵 (비용 절감, 초기 분석 데이터 사용)
  if(window.backtest&&backtest.running && !_evolveRunning && (sim.idx % 120 === 0) && sim.idx > 0){
...
async function execAutoBuy(pr, stk, score, techTag){
  ...
  // 가격 이상 감지: base 대비 50% 이상 차이나면 차단
  if(stk.base && pr > 0){
    var _diff = Math.abs(pr - stk.base) / stk.base;
    if(_diff > 0.5){
      addDecisionLog('['+stk.nm+'] ⚠ 가격 이상','실행가 '+fW(pr)+' vs 기준가 '+fW(stk.base)+' ('+Math.round(_diff*100)+'% 차이) — 매수 차단','NOGO');
      return;
    }
  }
  ...
  // 진화: 적응형 비중 + 기법 성과 반영
  var basePos=_adaptiveCfg.pos||30;
  let posPct;
  if(typeof score==='number'){
    if(score>=12) posPct=Math.round(basePos*1.5);
    else if(score>=10) posPct=Math.round(basePos*1.3);
    else if(score>=8) posPct=basePos;
    else posPct=Math.round(basePos*0.7);
  }else{
    posPct = Math.max(5, Math.min(80, basePos));
  }
  // 기법 성과 기반 조절
  var _tg=getTechGrade(techTag||'');
  if(_tg.grade==='A'){posPct=Math.round(posPct*1.15);addDecisionLog('['+stk.nm+'] A등급','비중 +15%','진화엔진');}
  else if(_tg.grade==='D'){posPct=Math.round(posPct*0.8);}
  ...
  if(qty<=0){ addDecisionLog('['+stk.nm+'] 잔고/신용 부족', '가용 '+totalAvail.toLocaleString()+'원 < 1주('+fW(pr)+')', '관망'); return; }
  ...
  var _adStop=_adaptiveCfg.stop||autoState.cfg.stop;
  var _adT1=_adaptiveCfg.t1||autoState.cfg.t1;
  var _adT2=_adaptiveCfg.t2||autoState.cfg.t2;
  document.getElementById("ofStop").value=Math.round(pr*(1-_adStop/100));
  document.getElementById("ofT1").value=Math.round(pr*(1+_adT1/100));
  document.getElementById("ofT2").value=Math.round(pr*(1+_adT2/100));
  ...
  if(ok){
    ...
    addDecisionLog(`[${stk.nm}] 자동 매수 (${posPct}% 비중)${_credTag}`,`${fW(pr)}원 × ${qty}주 = ${_inv}원 | 손절 ${Math.round(pr*(1-autoState.cfg.stop/100)).toLocaleString()} | 목표 ${Math.round(pr*(1+autoState.cfg.t1/100)).toLocaleString()}`,"Phase 9-4");
...
async function detectTechnique(cs, stk){
  ...
  // ── 진화: 다중 확인 보너스 ──
  var confirmBonus=0,confirmTags=[];
  // C1: 5일 평균 거래량 대비 (단순 전봉 비교 → 5일 평균)
  var avg5v_dt=vls.length>=6?(vls.slice(-6,-1).reduce(function(a,b){return a+b;},0)/5):vls[vls.length-2]||1;
  var volR5=vls[vls.length-1]/avg5v_dt;
  if(volR5>=2){confirmBonus+=2;confirmTags.push('거래량5일2배');}
  else if(volR5>=1.5){confirmBonus+=1;confirmTags.push('거래량5일1.5배');}
  // C2: MA 정배열 품질 (5>20>60)
  var ma60=calcMA(cls,60);var lma60_dt=ma60[ma60.length-1]||0;
  if(lma5>lma20&&lma20>lma60_dt&&lma60_dt>0){confirmBonus+=2;confirmTags.push('3중정배열');}
  else if(lma5>lma20){confirmBonus+=1;confirmTags.push('단기정배열');}
  // C3: 캔들 패턴 인식
  var bodySize=Math.abs(lc.c-lc.o);var totalRange=lc.h-lc.l||1;
  var upperW=lc.h-Math.max(lc.c,lc.o);var lowerW=Math.min(lc.c,lc.o)-lc.l;
  if(lc.c>lc.o&&bodySize/totalRange>=0.6&&lowerW<bodySize*0.3){confirmBonus+=1;confirmTags.push('장대양봉');}
  if(lc.c>lc.o&&lowerW>bodySize*2&&upperW<bodySize*0.5){confirmBonus+=1;confirmTags.push('망치형');}
  var pbody=Math.abs(pc.c-pc.o);var ptotal=pc.h-pc.l||1;
  if(pc.c<pc.o&&lc.c>lc.o&&lc.c>pc.o&&lc.o<pc.c){confirmBonus+=2;confirmTags.push('상승장악형');}
  // C4: RSI 다이버전스 (가격 하락 but RSI 상승)
  var rsiArr=calcRSI(cls,14);
  var rsi2=rsiArr[rsiArr.length-2]||rsi;
  if(cls[cls.length-1]<cls[cls.length-2]&&rsi>rsi2+2){confirmBonus+=1;confirmTags.push('RSI다이버전스');}
  // C5: VWAP 위치 확인
  if(cs.length>=10){
    var vwap=calcVWAP(cs.slice(-20));var lvwap=vwap[vwap.length-1]||0;
    if(lvwap>0&&lc.c>lvwap&&lc.c<lvwap*1.02){confirmBonus+=1;confirmTags.push('VWAP지지');}
  }
  // C6: 직전3봉 추세 (연속 양봉 or 저점 상승)
  if(cs.length>=4){
    var c3=cs.slice(-3);
    var risingLows=c3[0].l<c3[1].l&&c3[1].l<c3[2].l;
    if(risingLows){confirmBonus+=1;confirmTags.push('저점상승');}
  }
  score+=confirmBonus;
  // ── 진화: 성과 기반 점수 조절 ──
  var tg=getTechGrade(technique);
  if(tg.grade!=='?'){
    if(tg.grade==='D'){score=Math.round(score*0.5);confirmTags.push('D등급↓');}
    else if(tg.grade==='A'){score=Math.round(score*1.2);confirmTags.push('A등급↑');}
  }
  const t=TECHNIQUES[technique]||TECHNIQUES["눌림목"];
  const entryPr=lc.c;
  var adStop=_adaptiveCfg.stop||3;
  const stopPr=Math.round(Math.min(lma20*0.995,entryPr*(1-adStop/100))||entryPr*0.97);
  var adRR=_adaptiveCfg.rr||2;
  const rr=Math.max(adRR,1.5);
  const targetPr=Math.round(entryPr+(entryPr-stopPr)*rr);
  return {technique,score,phase:t.phase,cond:t.cond,entry:entryPr,stop:stopPr,target:targetPr,rr,ma5:Math.round(lma5),ma20:Math.round(lma20),rsi:rsi.toFixed(0),volRatio:volR5.toFixed(1),confirmTags:confirmTags,techGrade:tg.grade};
}
...
async function runFullAnalysis(){
  ...
  if(!hasClaudeKey){
    ...
    if(tech){
      ...
      if(ep2) ep2.innerHTML=
        `<span style="color:var(--r);font-weight:700;">📍 진입 ${fW(tech.entry||lc2.c)}원</span><br>`+
      ...
      if(ep2) ep2.innerHTML=
        `<span style="color:var(--r);font-weight:700;">📍 진입 ${fW(lc2.c)}원</span><br>`+
  ...
  var _perfSum=buildPerfSummary();
  var _techG=tech?getTechGrade(tech.technique):{grade:'?'};
  var _learnCtx=(window.learningMemory||[]).slice(-5).map(function(l){return'['+l.category+'] '+l.text;}).join('\n');
...
감지된 기법: ${tech?.technique||"불명확"} ${tech?.confirmTags?'['+tech.confirmTags.join(',')+']':''}
${_techG.grade!=='?'?`기법 과거성과: ${_techG.grade}등급 ${_techG.score}점/100 승률${_techG.winRate}% R/R${_techG.rr} PF${_techG.pf||'-'} ${_techG.count}건`:''}
${pos?`보유: ${pos.qty}주 평단 ${Math.round(pos.avgPrice).toLocaleString()}원${so?` 손절${fW(so.stop)} 목표${fW(so.t1)}`:""}`:""}

[적응형 파라미터] 손절${_adaptiveCfg.stop}% / 비중${_adaptiveCfg.pos}% / 최소R/R ${_adaptiveCfg.rr}
[전체 성과] ${_perfSum}
[패턴 DB] ${(function(){var c=captureEntryConditions();var q=queryPatternDB(c,tech?.technique);return q.msg;})()}
[원칙 준수 통계] ${getAdherenceInsight().msg}
[시장 상태] ${detectMarketRegime()} ${(function(){var r=detectMarketRegime();var a=getRegimeAdvice(r,tech?.technique||'');return a.msg;})()}
[최근 학습]
${_learnCtx||'없음'}

위 성과 데이터를 반드시 반영해서 GO/NO-GO 판단해줘:
- D등급 기법 → NO-GO (과거 데이터가 증명한 실패 기법)
- A등급 기법 → GO 확률 상향 + 비중 확대 가능
- R/R < 1.0 기법 → NO-GO (손익비 역전)
- PF < 0.7 기법 → NO-GO (총손실 > 총수익)
- 최근 학습에서 경고/실수 있는 종목 → 신중 진입
- 전체 성과 점수 30 미만 → 모든 진입 기준 상향
  ...
  try{
    ...
    if(p.entry){
      ...
      document.getElementById("entryPoint").innerHTML=
        `<span style="color:var(--b);font-weight:700;">진입 ${fW(p.entry)}원</span><br>`+
        `<span style="color:var(--r);">✂ 손절 ${fW(p.stop)}원</span><br>`+
        `<span style="color:var(--g);">🎯 1차 ${fW(p.target1)}원</span><br>`+
        `<span style="color:var(--g);">🎯 2차 ${fW(p.target2)}원</span>`+
    ...
    addDecisionLog(
      ...
      `진입 ${fW(p.entry)} / 손절 ${fW(p.stop)} / 목표 ${fW(p.target1)} · ${p.phase8?.step7_final||""}`,
...
function applyAnalysisResult(p){
  ...
  if(p.entry){
    ...
    if(ep)ep.innerHTML='<span style="color:var(--b);font-weight:700;">진입 '+fW(p.entry)+'원</span><br><span style="color:var(--r);">✂ 손절 '+fW(p.stop)+'원</span><br><span style="color:var(--g);">🎯 1차 '+fW(p.target1)+' | 2차 '+fW(p.target2)+'원</span><br><span style="font-size:8px;color:var(--ts);">R/R 1:'+(p.rr||'-')+'</span>';
  }
  const tag=document.getElementById('goNogoTag');
...
async function runAISectorAnalysis(){
  ...
  const lc = cs[cs.length-1]||{};
  const marketCtx = `현재시각 ${lc.t||'--'} | RSI ${curRSI} | 거래량비율 ${curVol}배 | 종가 ${fW(lc.c)}원`;
...
async function syncKisBalance() {
  ...
  try {
    ...
    addMsg('ai', `📊 KIS 잔고 동기화 완료\n예수금: ${fW(result.cash)}원 | 총평가: ${fW(result.totalEval)}원`);
  } catch(e) { /* 조용히 실패 */ }
}
...
손절선:${so?fW(so.stop)+"원":"미설정⚠"} | 1차목표:${so?fW(so.t1)+"원":"미설정"}
...
[STEP 6 | Phase 8-8 | R/R] 기준타점: 진입${fW(entrySug)} / 손절${fW(stopSug)}(-3%) / 목표${fW(tgt1Sug)}(+6%)
...
function addAlert(){
  ...
  showAlert("알림 설정","가격 알림이 설정되었습니다.\n"+
    `${STOCKS.find(s=>s.tk===activeTk)?.nm||activeTk} ${fW(pr)}원 ${dir==="above"?"이상":"이하"} 도달 시 알림`);
}
function renderAlerts(){
  ...
  el.innerHTML=priceAlerts.filter(a=>!a.fired).map((a,i)=>{
    ...
    return`<div style="display:flex;align-items:center;gap:4px;padding:2px 0;border-bottom:1px solid var(--br);">
      <span style="flex:1;">${stk.nm} ${fW(a.price)} ${a.dir==="above"?"↑":"↓"} ${a.memo||""}</span>
...
function renderPort(){
  ...
  c.innerHTML=keys.map(tk=>{
    ...
    const so=stopOrders[tk];
    const bgs=(pos.creditType&&pos.creditType!=="cash"?`<span class="pi-b cred">${pos.creditType==="credit"?"신용":"미수"}</span>`:"")+(so?`<span class="pi-b stop">손절${fW(so.stop)}</span>`:"")+(so&&!so.t1done?`<span class="pi-b tgt">목표${fW(so.t1)}</span>`:"")+(so?.trail!=="off"?`<span class="pi-b trail">트레일</span>`:"")+(pos.auto?`<span class="pi-b ai">AI</span>`:"");
  ...
  el.innerHTML=res.map(s=>{
    ...
    return`<div class="sri" onclick="selectStk('${s.tk}')">
      ...
      <div style="text-align:right;flex-shrink:0;">
        <div class="sri-pr ${up?"cu":"cd"}">${fW(s.pr)}</div>
...
function renderStats(){
  ...
  if(dates.length){
    ...
    if(pcv && typeof Chart !== 'undefined'){
      try{ const ctx=pcv.getContext("2d"); pnlChart=new Chart(ctx,{type:"line",data:{labels:dates,datasets:[{data:cd,borderColor:"#4d9fff",borderWidth:2,pointRadius:3,fill:true,backgroundColor:"rgba(77,159,255,.08)",tension:.3}]},options:{responsive:true,maintainAspectRatio:false,animation:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:"#454b58",font:{size:8}}},y:{ticks:{color:"#8c9db5",font:{size:8},callback:v=>fW(v)}}}}}); }catch(_e){ console.warn('Chart 실패:', _e.message); }
    } else if(pcv){
  ...
  renderSymbolPnl();
  renderTimeline();
  try{ renderTradeDetailLog(); }catch(e){}
  try{ renderLearningMgmt(); }catch(e){}
  // 주간/월간 통계 자동 호출
      ...
      const rows = _dates.map(d=>{
        ...
        return `<div style="display:flex;align-items:center;justify-content:space-between;padding:6px 10px;border-radius:6px;background:var(--bg);">
          ...
          <span style="display:flex;gap:12px;align-items:baseline;">
            <span style="font-family:var(--mono);font-size:12px;font-weight:700;color:${col};">${v>=0?'+':''}${fW(v)}원</span>
            <span style="font-size:9px;color:var(--tm);">누적</span>
            <span style="font-family:var(--mono);font-size:10px;color:${cumCol};">${cum>=0?'+':''}${fW(cum)}원</span>
          </span>
        </div>`;
      }).join('');
...
async function genJModal(){
  ...
  // ── 1. AI 없이도 즉시 기본 저장 ──
  const baseEntry = {
    summary: `${sim.date} 매매 (${total}건, ${pnl>=0?'+':''}${fW(pnl)}원)`,
    why_bought: '기록 없음',
    why_sold: '기록 없음',
    mistakes: '-',
    psychology: '-',
...
{"summary":"오늘 매매 한줄 총평","market_context":"오늘 시장 환경 평가(코스피/나스닥 방향 등)","why_bought":"진입 이유 분석 - Phase 8 STEP 기준 충족 여부","why_sold":"청산 이유 분석 - 손절/익절 원칙 준수 여부","mistakes":"가장 심각한 실수 2~3가지(구체적 수치와 함께)","psychology":"심리 상태 평가(FOMO/복수매매/조기익절 등 오류)","phase_check":"Phase 8 체크리스트 중 건너뛴 항목","improvement":"다음에 같은 상황에서 다르게 할 행동(구체적으로)","score_principle":8,"score_timing":7,"score_psychology":7,"score_total":7,"score_reason":"점수 이유"}`}]})});
    ...
    body.innerHTML=`<div class='jday'><div class='jd-hdr'><span class='jd-dt'>${sim.date}</span><span class='jd-pnl ${pnl>=0?"cu":"cd"}'>${pnl>=0?"+":""}${fW(pnl)}원</span></div>
...
function _renderJEntry(e){
  return `<div class="jday" style="margin-bottom:0;">
    <div class="jd-hdr"><span class="jd-dt">${e.date||''}</span><span class="jd-pnl ${(e.pnl||0)>=0?"cu":"cd"}">${(e.pnl||0)>=0?"+":""}${fW(e.pnl)}원</span></div>
    ...
    return `<div class="jday">
      <div class="jd-hdr">
        ...
        <span style="display:flex;align-items:center;gap:6px;">
          <span class="jd-pnl ${(e.pnl||0)>=0?"cu":"cd"}" style="font-size:14px;font-weight:800;">${(e.pnl||0)>=0?"+":""}${fW(e.pnl)}원</span>
...
function renderJournalStats(){
  ...
  if(kpiEl){
    ...
    kpiEl.innerHTML =
      kpi('총 손익', (d.totalPnl>=0?'+':'')+fW(d.totalPnl)+'원', pnlCol) +
  ...
  if(bd){
    ...
    const extras = `
      <div style="background:var(--bg);border-radius:8px;padding:10px;font-size:10px;line-height:1.7;">
        ...
        ${d.best?`<div>최고일: ${d.best.date} <b style="color:var(--g);">+${fW(d.best.pnl)}</b></div>`:''}
        ${d.worst?`<div>최악일: ${d.worst.date} <b style="color:var(--r);">${fW(d.worst.pnl)}</b></div>`:''}
      </div>`;
    const topStk = `
      <div style="background:var(--bg);border-radius:8px;padding:10px;grid-column:1/-1;">
        ...
        ${d.topStocks.slice(0,3).map(s=>`<div style="display:flex;justify-content:space-between;font-size:10px;padding:2px 0;"><span>${s.nm} (${s.tk}) · ${s.count}건 · 승${s.wins}</span><b style="color:${s.pnl>=0?'var(--g)':'var(--r)'};">${s.pnl>=0?'+':''}${fW(s.pnl)}원</b></div>`).join('')}
        ${d.topStocks.length>3 ? '<div style="font-size:9px;color:var(--tm);margin:4px 0;text-align:center;">···</div>' : ''}
        ${d.topStocks.slice(-2).reverse().filter(s=>!d.topStocks.slice(0,3).includes(s)).map(s=>`<div style="display:flex;justify-content:space-between;font-size:10px;padding:2px 0;"><span>${s.nm} (${s.tk}) · ${s.count}건 · 승${s.wins}</span><b style="color:${s.pnl>=0?'var(--g)':'var(--r)'};">${s.pnl>=0?'+':''}${fW(s.pnl)}원</b></div>`).join('')}
      </div>`;
...
【통계】
총손익 ${fW(s.totalPnl)}원 / 매매 ${s.sells.length}건 / 승률 ${s.winRate.toFixed(1)}%
손익비 1:${s.rr.toFixed(2)} / MDD -${Math.round(s.mdd).toLocaleString()}원 / 최대연패 ${s.maxLoss}회
AI손익 ${fW(s.aiPnl)} (승률 ${s.aiWinRate.toFixed(1)}%) vs 수동 ${fW(s.manPnl)} (${s.manWinRate.toFixed(1)}%)
...
async function runIdxAIPrediction(){
  ...
  try{
    const data=await callClaude({
      ...
      messages:[{role:'user',content:`지금 시장 초단기(5~15분) 방향 예측. RSI:${rsi} 거래량:${vol}배 현재가:${fW(lc.c)}. JSON만: {"dir":"상승예상"또는"하락예상"또는"횡보","confidence":75}`}]
    },'지수예측');
...
function copyAllJournals(){
  try{
    ...
    entries.forEach(e=>{
      md += `## ${e.date} (${(e.pnl||0)>=0?'+':''}${fW(e.pnl)}원, ${e.total||0}건)\n\n`;
...
const INTRA_ITEMS = [
  ...
  { label:'오늘 손익', icon:'💰', fn:()=>{
    ...
    return {score:ok?85:pct>-1?60:pct>-2?35:15, val:`${pnl>=0?'+':''}${fW(pnl)}원`, ok};
  }},
];
...
추천이유: ${c.why||'조건검색 통과'} | 현재가: ${fW(lc.c)}원 | RSI: ${rsi}
  ...
  heldPositions.forEach(function(e){
    ...
    if(!stopOrders[tk]){
      ...
      if(stk && pos.avg){
        ...
        issues.push(stk.nm+' 손절/익절 미설정 → 자동 설정 (손절 '+fW(stopPr)+')');
      }
    }
  });
  ...
  try{
    ...
    result.sectors.forEach(function(sec){
      (sec.stocks||[]).forEach(function(stk){
        // 종목코드 정규화: 4~5자리도 허용, 6자리로 패딩
        let _tk = String(stk.tk||'').trim().replace(/[^\d]/g,'');
        while(_tk.length>0 && _tk.length<6) _tk = '0'+_tk;
...
async function _autoSaveJournalOnTradeInner(date){
  ...
  const str=tt.map(function(t){return (t.side==='buy'?'매수':'매도')+' '+t.nm+' '+t.qty+'주 @'+fW(t.price)+(t.pnl!==undefined?' ('+(t.pnl>=0?'+':'')+Math.round(t.pnl).toLocaleString()+'원)':'');}).join('\n');
  ...
  const baseEntry={
    summary: date+' '+_resultGrade+' | '+tt.length+'건 '+(pnl>=0?'+':'')+fW(pnl)+'원 (승률 '+_winRate+'%)',
  ...
  try{
    ...
    const prompt='단타 트레이딩 멘토. 오늘 매매를 강의 원칙 기준으로 평가해서 매매일지 작성.\n\n'+_lecJ+
      '\n날짜: '+date+
      '\n결과: '+_resultGrade+' / 손익 '+(pnl>=0?'+':'')+fW(pnl)+'원 / 승률 '+_winRate+'% ('+wins+'/'+total+')'+
      '\n\n매매내역:\n'+str+
      '\n\nAI결정로그:\n'+(logs||'없음')+
      '\n\n[평가 원칙 — 반드시 지킬 것]\n'+
...
function _showThought_buy(n,p,sp,t1,rr,cf,r){updAIThought({type:'buy',title:'매수 결정 - '+n,trend:{type:'ok',label:'진입근거:',text:r},entry:{type:'ok',label:'타점:',text:fW(p)+'원 신뢰도 '+cf+'%'},risk:{type:'info',label:'리스크:',text:'손절 '+fW(sp)+' 목표 '+fW(t1)+' R/R 1:'+rr},decision:{type:'ok',label:'결정:',text:'자동 매수 실행'},conclusion:'매수 - '+n+' '+fW(p)+'원 ('+cf+'%)'});}
...
function resetBalance(){
  ...
  if(!confirm(fW(amt)+"원으로 리셋하시겠습니까?"))return;
  ...
  addMsg("ai","잔고 "+fW(amt)+"원으로 리셋 완료");
  showAlert("리셋 완료",fW(amt)+"원으로 리셋되었습니다.");
}
