...
// ═══════════════════════════════
function W(v){return Math.round(v||0).toLocaleString();}
...
[TraidAIr 상태 진단]
━━━━━━━━━━━━━━━━━━━━━━
💰 자본: 현금 ${W(mock.cash)}원 / 보유 ${Object.keys(mock.positions||{}).length}종목
...
function renderPending(){const el=document.getElementById("pendingList");if(!el)return;if(!pendingOrders.length){el.innerHTML="<div style='font-size:9px;color:var(--tm);padding:8px;'>미체결 없음</div>";return;}el.innerHTML=pendingOrders.map(o=>`<div style="padding:4px 8px;border-bottom:1px solid var(--br);display:flex;align-items:center;gap:4px;"><span style="font-size:9px;font-weight:700;color:${o.side==="buy"?"var(--g)":"var(--r)"};">${o.side==="buy"?"매수":"매도"}</span><span style="font-size:9px;flex:1;">${o.nm} ${W(o.price)}×${o.qty}</span><span style="font-size:8px;color:var(--tm);">${o.time}</span><button onclick="cancelPending(${o.id})" style="background:var(--r);color:#fff;border:none;border-radius:3px;font-size:8px;padding:1px 4px;cursor:pointer;">취소</button></div>`).join("");}
...
function _renderRealOrderbook(d){
  ...
  [...asks].reverse().forEach(a => {
    ...
    sell += `<div class="ob-r" onclick="document.getElementById('ofPr').value=${a.price};updOSum()"><div class="ob-bg s" style="width:${w}%"></div><div class="ob-p" style="color:#2563eb;">${W(a.price)}</div><div class="ob-q">${a.qty.toLocaleString()}</div></div>`;
  });

  let buy = '';
  bids.forEach(b => {
    const w = Math.min(100, (b.qty/maxQ*100)).toFixed(0);
    buy += `<div class="ob-r" onclick="document.getElementById('ofPr').value=${b.price};updOSum()"><div class="ob-bg b" style="width:${w}%"></div><div class="ob-p" style="color:#dc2626;">${W(b.price)}</div><div class="ob-q">${b.qty.toLocaleString()}</div></div>`;
  });
...
function checkPending(){pendingOrders.forEach((o,i)=>{const stk=STOCKS.find(s=>s.tk===o.tk);if(!stk)return;const hit=(o.side==="buy"&&stk.pr<=o.price)||(o.side==="sell"&&stk.pr>=o.price);if(hit){const prev=activeTk,prevSide=oSide,prevType=oType;activeTk=o.tk;oSide=o.side;oType="market";document.getElementById("ofQty").value=o.qty;submitOrder(true);activeTk=prev;oSide=prevSide;oType=prevType;pendingOrders.splice(i,1);addMsg("ai",`✅ 지정가 체결: ${o.nm} ${o.side==="buy"?"매수":"매도"} ${W(o.price)}원 × ${o.qty}주`);renderPending();}});}
let pendingOrders=[];
...
function _addLearningReply(stockNm, pnl, tk){
  ...
  const pnlStr=pnl>=0?'+'+W(pnl):W(pnl);
  const icon=pnl>=0?'✅':'❌';
...
async function _backtestEndOfDayInner(dt){
  ...
  backtest.dayIdx++;
  addDecisionLog(`📅 ${dt} 마감`, `매매 ${todayTrades.length}건 | 손익 ${dayPnl>=0?'+':''}${W(dayPnl)}원 | 승 ${wins}·패 ${losses}`, '백테스트');
  renderBacktestPanel();
  ...
  try{
    if(todayTrades.length > 0){
      ...
      const _baseJ = {
        summary: dt+' '+_rg+' | '+todayTrades.length+'건 '+(dayPnl>=0?'+':'')+W(dayPnl)+'원 (승률 '+_wr+'%)',
        result_grade: _rg,
        ...
        aiGenerated: false,
        trades: todayTrades.map(t=>(t.side==='buy'?'매수':'매도')+' '+t.nm+' '+t.qty+'주 @'+W(t.price)+(t.pnl?(' ('+(t.pnl>=0?'+':'')+Math.round(t.pnl).toLocaleString()+'원)'):'')).join('\n'),
      };
  ...
  el.innerHTML = '<span style="color:var(--tm);font-weight:700;margin-right:6px;">⚡ 실시간 매매</span>' +
    recent.map(function(t){
      ...
      return `<span style="display:inline-block;margin-right:10px;padding:1px 4px;border-radius:3px;background:var(--pan);border:1px solid var(--br);">
        ...
        <span style="color:var(--ts);">${W(t.price||t.pr)}원×${t.qty}</span>
        ${pnlTxt}
      </span>`;
    }).join('');
}
...
function renderBacktestPanel(){
  ...
  el.innerHTML = `
    <div style="display:flex;align-items:center;gap:8px;">
      ...
      <span style="font-family:var(--mono);font-size:10px;font-weight:700;color:${realtimePnl>=0?'var(--g)':'var(--r)'};">${realtimePnl>=0?'+':''}${W(realtimePnl)}원</span>
...
function updPrice(c){
  ...
  if(siPr){siPr.textContent=W(pr);siPr.className="si-pr "+cls;}
  ...
  const chPr=document.getElementById("chartHdrPr");if(chPr) chPr.textContent=W(pr);
  ...
  if(obPr){obPr.textContent=W(pr);obPr.className="ob-cur-p "+cls;}
...
function renderOB(){
  ...
  for(let i=10;i>=1;i--){
    ...
    sellRows.push(`<div class="ob-r" onclick="document.getElementById('ofPr').value=${pr};updOSum()"><div class="ob-bg s" style="width:${w}%"></div><div class="ob-p" style="color:#2563eb;">${W(pr)}</div><div class="ob-q">${q.toLocaleString()}</div></div>`);
  }
  // 매수 10호가 (국내 표준: 매수=빨강)
  const buyRows=[];
  for(let i=1;i<=10;i++){
    ...
    totalBuy+=q;
    const w=Math.min(100,(q/maxQ*100)).toFixed(0);
    buyRows.push(`<div class="ob-r" onclick="document.getElementById('ofPr').value=${pr};updOSum()"><div class="ob-bg b" style="width:${w}%"></div><div class="ob-p" style="color:#dc2626;">${W(pr)}</div><div class="ob-q">${q.toLocaleString()}</div></div>`);
  }
  // 총잔량 헤더 (매도)
...
function renderTH(){
  ...
  for(let i=0;i<25;i++){
    ...
    rows.push(`<div style="display:flex;align-items:center;padding:2px 8px;border-bottom:1px solid var(--br);gap:5px;">
      ...
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;flex:1;text-align:right;color:${col};">${arrow} ${W(pr)}</span>
...
function onCredChange(){
  ...
  else{info.style.display="block";const avail=getCredAvail();info.textContent=credType==="credit"?`신용 가능: ${W(avail)}원 (이자 연 ${cfg.crate}%)`:`미수 가능: ${W(avail)}원 (이자 연 ${cfg.mrate}% | 미결제→반대매매)`;}
  updOSum();
}
...
function updOSum(){
  ...
  document.getElementById("ofSum").innerHTML=`예상: <b>${W(tot)}</b>원 | 비용 ${Math.round(fee+tax).toLocaleString()}원${rr}${credType!=="cash"?` | <span style="color:var(--a)">${credType==="credit"?"신용":"미수"}</span>`:""}`;
  ...
  if(oSide==="sell"&&mock.positions[activeTk]&&pr&&qty){
    ...
    if(pnlEl)pnlEl.innerHTML+=` | 실질 <b style="color:${netPnl>=0?"var(--g)":"var(--r)"}">${netPnl>=0?"+":""}${W(netPnl)}원</b>`;
  }
}
...
function checkStopOrders(){
  ...
  Object.entries(stopOrders).forEach(([tk,so])=>{
    ...
    if(pr<=so.stop&&pos.qty>0){
      ...
      addMsg("ai",`🔴 손절 실행! ${stk.nm} ${W(pr)}원\n손절가 ${W(so.stop)}원 도달 → 전량 청산\n[Phase 9-4: 손절은 시장가 즉시]`);
      return;
    }
    // T1 (50%)
    if(!so.t1done&&pr>=so.t1&&pos.qty>0){
      ...
      addMsg("ai",`🟢 1차 익절! ${stk.nm} ${W(pr)}원 (50% 청산)\n손절선 본전화 적용\n[Phase 9-2: 분할 익절]`);
      renderPort();
    }
    // T2 (30%)
    if(so.t1done&&!so.t2done&&pr>=so.t2&&pos.qty>0){
      ...
      so.t2done=true;
      addMsg("ai",`🟢 2차 익절! ${stk.nm} ${W(pr)}원 (30% 청산)\n잔여 20% 트레일링 스탑 유지\n[Phase 9-2]`);
      renderPort();
    }
  });
  // Phase 3-8: 가격 알림 체크
...
평균 수익: ${W(avgWin)}원 | 평균 손실: ${W(avgLoss)}원
총 손익: ${totalPnl>=0?'+':''}${W(totalPnl)}원`;
  const winDetail=wins.length?wins.slice(-15).map(t=>`+${Math.round(t.pnl||0).toLocaleString()}원 ${t.nm} ${W(t.price)}원 (${t.time||t.barTime||''}) ${t.auto?'AI':'수동'}`).join('\n'):'(수익 거래 없음)';
  const lossDetail=losses.length?losses.slice(-15).map(t=>`${Math.round(t.pnl||0).toLocaleString()}원 ${t.nm} ${W(t.price)}원 (${t.time||t.barTime||''}) ${t.auto?'AI':'수동'}`).join('\n'):'(손실 거래 없음)';
  const byStock={};
  trades.forEach(t=>{
    ...
    if(!wins.length){wEl.innerHTML="<span style='color:var(--tm);'>수익 거래 없음</span>";}
      ...
      wEl.innerHTML=`<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px;">
        <div style="background:rgba(5,192,114,.06);border-radius:8px;padding:8px;text-align:center;">
          ...
          <div style="font-family:var(--mono);font-size:13px;font-weight:700;color:var(--g);">+${W(avgWin)}원</div>
        </div>
        <div style="background:rgba(5,192,114,.06);border-radius:8px;padding:8px;text-align:center;">
          <div style="font-size:9px;color:var(--tm);">최대 수익</div>
          <div style="font-family:var(--mono);font-size:13px;font-weight:700;color:var(--g);">+${W(maxWin)}원</div>
        </div>
      </div>
    ...
    if(!losses.length){lEl.innerHTML="<span style='color:var(--tm);'>손실 거래 없음</span>";}
      ...
      lEl.innerHTML=`<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px;">
        <div style="background:rgba(240,62,62,.06);border-radius:8px;padding:8px;text-align:center;">
          ...
          <div style="font-family:var(--mono);font-size:13px;font-weight:700;color:var(--r);">${W(avgLoss)}원</div>
        </div>
        <div style="background:rgba(240,62,62,.06);border-radius:8px;padding:8px;text-align:center;">
          <div style="font-size:9px;color:var(--tm);">최대 손실</div>
          <div style="font-family:var(--mono);font-size:13px;font-weight:700;color:var(--r);">${W(maxLoss)}원</div>
        </div>
      </div>`;
    }
  }
...
function checkPsychWarnings(){
  ...
  Object.entries(mock.positions).forEach(([tk,pos])=>{
    ...
    if(pnlPct>0&&pnlPct<1&&so&&stk.pr<so.t1){
      warnings.push(`처분효과 주의: ${stk.nm} +${pnlPct.toFixed(1)}% — 목표가(${W(so.t1)}) 미달. 조기청산 충동 = 처분효과`);
    }
    ...
    ${trades.map(t=>{
      ...
      return`<div style="position:relative;padding:5px 0 5px 12px;margin-bottom:2px;">
        ...
        <div style="display:flex;align-items:center;gap:6px;">
          ...
          <span style="font-family:var(--mono);font-size:9px;color:var(--ts);">${W(t.price)}원 × ${t.qty}주</span>
      ...
      ${pairs.map(p=>{
        ...
        return`<div style="padding:4px 0;border-top:1px solid var(--br);">
          ...
          <div style="font-size:9px;color:var(--ts);margin-top:2px;">
            ${p.buy?`<span style="color:var(--b);">▲ 진입</span> ${W(p.buy.price)}원 (${p.buy.time||'-'}) ${p.buy.auto?'🤖':'👤'}`:''}
            → <span style="color:var(--r);">▼ 청산</span> ${W(p.sell.price)}원 (${p.sell.time||'-'})
          </div>
        </div>`;
      }).join('')}
...
const REVIEW_PROMPTS = {
  1: (trades, pnl) => `아래 매매를 Phase 8~12 기준으로 분석해줘.\n\n매매: ${trades.slice(-1).map(t=>`${t.nm} ${t.side==='buy'?'매수':'매도'} ${W(t.price)}원 ${t.qty}주 손익${t.pnl>=0?'+':''}${Math.round(t.pnl||0)}원`).join(', ')}\n\n평가:\n1. 진입 타점 적절성 (Phase 8)\n2. 청산 타이밍 (Phase 9)\n3. R/R 수준\n4. 원칙 준수 여부\n5. 반복 실수 여부\n6. 같은 상황 다시 오면?`,
...
function updateTrailPanel(){
  ...
  el.innerHTML=`<div style="color:${up?"var(--g)":"var(--r)"};font-weight:700;">${up?"+":""}${pnlPct}%</div>`+
    ...
    `<div style="color:var(--r);">손절 ${W(so.stop)}</div>`+
    (so.t1?`<div style="color:var(--g);">1차 ${W(so.t1)} ${so.t1done?"✅":""}</div>`:``)+ 
  ...
  if(autoState.level>=3){
    try{
      ...
      var prompt='단타 트레이딩 멘토. 자동매매 최종 진입 여부.\n\n'+
        ...
        '【기술적】 가격 '+W(lc2.c)+'원 | RSI '+best.lrsi+' | 거래량 평균x'+best.volR.toFixed(1)+'\n'+
        '【조건】 '+best.tags.join(' · ')+'\n'+
        (_sectorLine ? '【수급/섹터】 '+_sectorLine+'\n' : '')+
        (_shortLine ? '【공매도】 '+_shortLine+'\n' : '')+
        (_newsItems ? '【최근 공시】\n'+_newsItems+'\n' : '')+
        (_mkt && _mkt!=='데이터없음' ? '【시황】\n'+_mkt+'\n' : '')+
        '【리스크】 손절 '+W(stopPr)+' | 목표 '+W(t1Pr)+' | R/R 1:'+rr+'\n\n'+
      ...
      if(ai.decision==='BUY'&&ai.confidence>=65){
        ...
        addMsg('ai','🤖 AI자동매수\n\n종목: '+best.stk.nm+' '+W(lc2.c)+'원\n신뢰도: '+ai.confidence+'%\n\n✅ '+ai.reason+'\n'+_factorsTxt+'⚠ '+ai.risk+'\n\n손절: '+W(stopPr)+' | 목표: '+W(t1Pr)+' | R/R 1:'+rr);
    ...
    updAdvBoxes('📊 진입신호 — '+best.stk.nm,best.tags.join(' · ')+'\n진입: '+W(lc3.c)+' 손절: '+W(stopPr2)+' 목표: '+W(t1Pr2)+' R/R:1:'+rr2+'\n⚡ 직접 주문창에서 실행');
...
async function execAutoBuy(pr, stk, score, techTag){
  ...
  if(qty<=0){ addDecisionLog('['+stk.nm+'] 잔고/신용 부족', '가용 '+totalAvail.toLocaleString()+'원 < 1주('+W(pr)+')', '관망'); return; }
  ...
  if(ok){
    ...
    addDecisionLog(`[${stk.nm}] 자동 매수 (${posPct}% 비중)${_credTag}`,`${W(pr)}원 × ${qty}주 = ${_inv}원 | 손절 ${Math.round(pr*(1-autoState.cfg.stop/100)).toLocaleString()} | 목표 ${Math.round(pr*(1+autoState.cfg.t1/100)).toLocaleString()}`,"Phase 9-4");
  ...
  if(!hasClaudeKey){
    ...
    if(tech){
      ...
      if(ep2) ep2.innerHTML=
        `<span style="color:var(--r);font-weight:700;">📍 진입 ${W(tech.entry||lc2.c)}원</span><br>`+
      ...
      if(ep2) ep2.innerHTML=
        `<span style="color:var(--r);font-weight:700;">📍 진입 ${W(lc2.c)}원</span><br>`+
...
${pos?`보유: ${pos.qty}주 평단 ${Math.round(pos.avgPrice).toLocaleString()}원${so?` 손절${W(so.stop)} 목표${W(so.t1)}`:""}`:""} 
  ...
  try{
    ...
    if(p.entry){
      ...
      document.getElementById("entryPoint").innerHTML=
        `<span style="color:var(--b);font-weight:700;">진입 ${W(p.entry)}원</span><br>`+
        `<span style="color:var(--r);">✂ 손절 ${W(p.stop)}원</span><br>`+
        `<span style="color:var(--g);">🎯 1차 ${W(p.target1)}원</span><br>`+
        `<span style="color:var(--g);">🎯 2차 ${W(p.target2)}원</span>`+
    ...
    addDecisionLog(
      ...
      `진입 ${W(p.entry)} / 손절 ${W(p.stop)} / 목표 ${W(p.target1)} · ${p.phase8?.step7_final||""}`,
...
function applyAnalysisResult(p){
  ...
  if(p.entry){
    ...
    if(ep)ep.innerHTML='<span style="color:var(--b);font-weight:700;">진입 '+W(p.entry)+'원</span><br><span style="color:var(--r);">✂ 손절 '+W(p.stop)+'원</span><br><span style="color:var(--g);">🎯 1차 '+W(p.target1)+' | 2차 '+W(p.target2)+'원</span><br><span style="font-size:8px;color:var(--ts);">R/R 1:'+(p.rr||'-')+'</span>';
  }
  const tag=document.getElementById('goNogoTag');
...
async function runAISectorAnalysis(){
  ...
  const lc = cs[cs.length-1]||{};
  const marketCtx = `현재시각 ${lc.t||'--'} | RSI ${curRSI} | 거래량비율 ${curVol}배 | 종가 ${W(lc.c)}원`;
...
async function syncKisBalance() {
  ...
  try {
    ...
    addMsg('ai', `📊 KIS 잔고 동기화 완료\n예수금: ${W(result.cash)}원 | 총평가: ${W(result.totalEval)}원`);
  } catch(e) { /* 조용히 실패 */ }
}
...
손절선:${so?W(so.stop)+"원":"미설정⚠"} | 1차목표:${so?W(so.t1)+"원":"미설정"}
...
[STEP 6 | Phase 8-8 | R/R] 기준타점: 진입${W(entrySug)} / 손절${W(stopSug)}(-3%) / 목표${W(tgt1Sug)}(+6%)
...
function addAlert(){
  ...
  showAlert("알림 설정","가격 알림이 설정되었습니다.\n"+
    `${STOCKS.find(s=>s.tk===activeTk)?.nm||activeTk} ${W(pr)}원 ${dir==="above"?"이상":"이하"} 도달 시 알림`);
}
function renderAlerts(){
  ...
  el.innerHTML=priceAlerts.filter(a=>!a.fired).map((a,i)=>{
    ...
    return`<div style="display:flex;align-items:center;gap:4px;padding:2px 0;border-bottom:1px solid var(--br);">
      <span style="flex:1;">${stk.nm} ${W(a.price)} ${a.dir==="above"?"↑":"↓"} ${a.memo||""}</span>
...
function renderPort(){
  ...
  c.innerHTML=keys.map(tk=>{
    ...
    const so=stopOrders[tk];
    const bgs=(pos.creditType&&pos.creditType!=="cash"?`<span class="pi-b cred">${pos.creditType==="credit"?"신용":"미수"}</span>`:"")+(so?`<span class="pi-b stop">손절${W(so.stop)}</span>`:"")+(so&&!so.t1done?`<span class="pi-b tgt">목표${W(so.t1)}</span>`:"")+(so?.trail!=="off"?`<span class="pi-b trail">트레일</span>`:"")+(pos.auto?`<span class="pi-b ai">AI</span>`:"");
...
function renderStats(){
  ...
  if(dates.length){
    ...
    if(pcv && typeof Chart !== 'undefined'){
      try{ const ctx=pcv.getContext("2d"); pnlChart=new Chart(ctx,{type:"line",data:{labels:dates,datasets:[{data:cd,borderColor:"#4d9fff",borderWidth:2,pointRadius:3,fill:true,backgroundColor:"rgba(77,159,255,.08)",tension:.3}]},options:{responsive:true,maintainAspectRatio:false,animation:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:"#454b58",font:{size:8}}},y:{ticks:{color:"#8c9db5",font:{size:8},callback:v=>W(v)}}}}}); }catch(_e){ console.warn('Chart 실패:', _e.message); }
    } else if(pcv){
      ...
      const rows = _dates.map(d=>{
        ...
        return `<div style="display:flex;align-items:center;justify-content:space-between;padding:6px 10px;border-radius:6px;background:var(--bg);">
          ...
          <span style="display:flex;gap:12px;align-items:baseline;">
            <span style="font-family:var(--mono);font-size:12px;font-weight:700;color:${col};">${v>=0?'+':''}${W(v)}원</span>
            <span style="font-size:9px;color:var(--tm);">누적</span>
            <span style="font-family:var(--mono);font-size:10px;color:${cumCol};">${cum>=0?'+':''}${W(cum)}원</span>
          </span>
        </div>`;
      }).join('');
...
async function genJModal(){
  ...
  // ── 1. AI 없이도 즉시 기본 저장 ──
  const baseEntry = {
    summary: `${sim.date} 매매 (${total}건, ${pnl>=0?'+':''}${W(pnl)}원)`,
    why_bought: '기록 없음',
    why_sold: '기록 없음',
    mistakes: '-',
    psychology: '-',
...
{"summary":"오늘 매매 한줄 총평","market_context":"오늘 시장 환경 평가(코스피/나스닥 방향 등)","why_bought":"진입 이유 분석 - Phase 8 STEP 기준 충족 여부","why_sold":"청산 이유 분석 - 손절/익절 원칙 준수 여부","mistakes":"가장 심각한 실수 2~3가지(구체적 수치와 함께)","psychology":"심리 상태 평가(FOMO/복수매매/조기익절 등 오류)","phase_check":"Phase 8 체크리스트 중 건너뛴 항목","improvement":"다음에 같은 상황에서 다르게 할 행동(구체적으로)","score_principle":8,"score_timing":7,"score_psychology":7,"score_total":7,"score_reason":"점수 이유"}`}]})});
    ...
    body.innerHTML=`<div class='jday'><div class='jd-hdr'><span class='jd-dt'>${sim.date}</span><span class='jd-pnl ${pnl>=0?"cu":"cd"}'>${pnl>=0?"+":""}${W(pnl)}원</span></div>
...
function _renderJEntry(e){
  return `<div class="jday" style="margin-bottom:0;">
    <div class="jd-hdr"><span class="jd-dt">${e.date||''}</span><span class="jd-pnl ${(e.pnl||0)>=0?"cu":"cd"}">${(e.pnl||0)>=0?"+":""}${W(e.pnl)}원</span></div>
    ...
    return `<div class="jday">
      <div class="jd-hdr">
        ...
        <span style="display:flex;align-items:center;gap:6px;">
          <span class="jd-pnl ${(e.pnl||0)>=0?"cu":"cd"}" style="font-size:14px;font-weight:800;">${(e.pnl||0)>=0?"+":""}${W(e.pnl)}원</span>
...
function updatePhase8Live(){
  ...
  el.innerHTML =
    ...
    row('STEP7 손절가', so ? `${W(so.stop)}원` : '미설정', stopOk);
}
...
async function runIdxAIPrediction(){
  ...
  try{
    const data=await callClaude({
      ...
      messages:[{role:'user',content:`지금 시장 초단기(5~15분) 방향 예측. RSI:${rsi} 거래량:${vol}배 현재가:${W(lc.c)}. JSON만: {"dir":"상승예상"또는"하락예상"또는"횡보","confidence":75}`}]
    },'지수예측');
...
function copyAllJournals(){
  try{
    ...
    entries.forEach(e=>{
      md += `## ${e.date} (${(e.pnl||0)>=0?'+':''}${W(e.pnl)}원, ${e.total||0}건)\n\n`;
...
const INTRA_ITEMS = [
  ...
  { label:'오늘 손익', icon:'💰', fn:()=>{
    ...
    return {score:ok?85:pct>-1?60:pct>-2?35:15, val:`${pnl>=0?'+':''}${W(pnl)}원`, ok};
  }},
];
...
추천이유: ${c.why||'조건검색 통과'} | 현재가: ${W(lc.c)}원 | RSI: ${rsi}
  ...
  heldPositions.forEach(function(e){
    ...
    if(!stopOrders[tk]){
      ...
      if(stk && pos.avg){
        ...
        issues.push(stk.nm+' 손절/익절 미설정 → 자동 설정 (손절 '+W(stopPr)+')');
      }
    }
  });
...
async function _autoSaveJournalOnTradeInner(date){
  ...
  const str=tt.map(function(t){return (t.side==='buy'?'매수':'매도')+' '+t.nm+' '+t.qty+'주 @'+W(t.price)+(t.pnl!==undefined?' ('+(t.pnl>=0?'+':'')+Math.round(t.pnl).toLocaleString()+'원)':'');}).join('\n');
  ...
  const baseEntry={
    summary: date+' '+_resultGrade+' | '+tt.length+'건 '+(pnl>=0?'+':'')+W(pnl)+'원 (승률 '+_winRate+'%)',
  ...
  try{
    ...
    const prompt='단타 트레이딩 멘토. 오늘 매매를 강의 원칙 기준으로 평가해서 매매일지 작성.\n\n'+_lecJ+
      '\n날짜: '+date+
      '\n결과: '+_resultGrade+' / 손익 '+(pnl>=0?'+':'')+W(pnl)+'원 / 승률 '+_winRate+'% ('+wins+'/'+total+')'+
      '\n\n매매내역:\n'+str+
      '\n\nAI결정로그:\n'+(logs||'없음')+
      '\n\n[평가 원칙 — 반드시 지킬 것]\n'+
...
function _showThought_buy(n,p,sp,t1,rr,cf,r){updAIThought({type:'buy',title:'매수 결정 - '+n,trend:{type:'ok',label:'진입근거:',text:r},entry:{type:'ok',label:'타점:',text:W(p)+'원 신뢰도 '+cf+'%'},risk:{type:'info',label:'리스크:',text:'손절 '+W(sp)+' 목표 '+W(t1)+' R/R 1:'+rr},decision:{type:'ok',label:'결정:',text:'자동 매수 실행'},conclusion:'매수 - '+n+' '+W(p)+'원 ('+cf+'%)'});}
...
function resetBalance(){
  ...
  if(!confirm(W(amt)+"원으로 리셋하시겠습니까?"))return;
  ...
  addMsg("ai","잔고 "+W(amt)+"원으로 리셋 완료");
  showAlert("리셋 완료",W(amt)+"원으로 리셋되었습니다.");
}
