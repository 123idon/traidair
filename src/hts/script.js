...
async function _runScreeningAsync(){
  ...
  for(var _ti=0;_ti<allTks.length;_ti++){
    ...
    if(kisConfig.appKey&&kisConfig.appSecret&&tk!==activeTk){
      try{
        ...
        if(_dd.ok&&_dd.candles&&_dd.candles.length>=10) cs=_dd.candles;
      }catch(_e){}
    }
    // 현재 활성 종목은 sim.candles 사용
    if(!cs.length){
      if(tk===activeTk) cs=getCandles(20);
      else continue; // KIS 없으면 현재 종목만
    }
    ...
    // 하락추세 차단
    if(lc.c<lma5&&lma5<lma20) continue;
    if(lc.c<lc.o&&pc.c<pc.o&&pc2.c<pc2.o) continue;

    var score=0,tags=[];
    if(lma5>lma20&&lma20>lma60){score+=3;tags.push('MA정배열');}
  ...
  if(!scored.length){
    addDecisionLog('전종목 조건미달','관심종목 모두 하락추세 또는 데이터없음','관망');
    return;
  }
  ...
  if(best.score<3){
    addDecisionLog('['+best.stk.nm+'] 관망',best.tags.join(' ')||'조건미달','관망');
    updAdvBoxes('관망 — '+best.stk.nm+' ('+best.score+'점)','기술적 조건 미달');
    return;
  }
...
// ── API 키 만료 감지 ──
function checkApiKeyExpiry(errorMsg){
  if(!errorMsg) return;
  const expired = errorMsg.includes('expired') || errorMsg.includes('invalid') ||
                  errorMsg.includes('401') || errorMsg.includes('authentication') ||
                  errorMsg.includes('Unauthorized') || errorMsg.includes('invalid_api_key');
  if(expired){
    const box = document.createElement('div');
    box.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;background:#dc2626;color:#fff;padding:10px 16px;font-size:13px;font-weight:700;display:flex;align-items:center;gap:10px;';
    box.innerHTML = '⚠️ API 키가 만료됐어요. 설정창에서 키를 새로 입력해주세요. <button onclick="openSettings();this.parentNode.remove()" style="border:2px solid #fff;background:none;color:#fff;padding:3px 12px;border-radius:5px;cursor:pointer;font-weight:700;">설정 열기</button><button onclick="this.parentNode.remove()" style="border:none;background:none;color:#fff;cursor:pointer;font-size:16px;margin-left:auto;">✕</button>';
...
window.onload=()=>{
  ...
  setInterval(refreshWatchlistPrices, 5000); // 관심종목 가격 5초마다 순차 갱신
  ...
  setTimeout(()=>{updChartHeader&&updChartHeader();renderHotSectors&&renderHotSectors();refreshHotSectors&&refreshHotSectors(false);}, 400);
...
function renderHotSectors(){
  ...
  // sectorInfo: {tk:{rank,sector,reason,nm}}
  const grouped={};
  Object.entries(info).forEach(([tk,v])=>{
    if(!grouped[v.rank])grouped[v.rank]={sector:v.sector,reason:v.reason,stocks:[]};
    grouped[v.rank].stocks.push({tk,nm:v.nm||tk});
  });
  ...
  el.innerHTML=ranks.map(r=>{
    const g=grouped[r];
    const stkLine=g.stocks.map(s=>'<span class="hot-sec-stk" data-tk="'+s.tk+'" onclick="selectStk(this.dataset.tk)" style="cursor:pointer;color:var(--b);font-weight:600;">'+s.nm+'</span>').join(' · ');
    return '<div style="line-height:1.4;"><span style="background:var(--r);color:#fff;border-radius:3px;padding:0 4px;font-weight:700;font-size:8px;margin-right:4px;">'+r+'위</span><span style="font-weight:700;">'+(g.sector||'-')+'</span><br><span style="color:var(--ts);font-size:8px;">'+(g.reason||'')+'</span><br>'+stkLine+'</div>';
  }).join('<div style="height:1px;background:var(--br);margin:3px 0;"></div>');
...
async function syncCandidatesToWatchlist(){
  try{
    ...
    // Claude AI로 당일 강세섹터 1~3위 + 대장주 분석
    const date = sim.date || new Date().toISOString().slice(0,10);
    const prompt = '오늘('+date+') 한국 주식시장 실시간 강세섹터 분석.\n\n'+
      '강세섹터 상위 3개와 각 섹터 대장주 1~2개 선정.\n'+
      '실적, 이슈, 수급 기반으로 판단.\n\n'+
      'JSON만 반환 (다른 텍스트 없이):\n'+
      '{"sectors":[{"rank":1,"name":"섹터명","reason":"강세이유1줄","stocks":[{"tk":"종목코드6자리","nm":"종목명","reason":"선정이유"}]},'+
      '{"rank":2,...},{"rank":3,...}]}';
    ...
    result.sectors.forEach(function(sec){
      (sec.stocks||[]).forEach(function(stk){
        if(/^\d{6}$/.test(stk.tk) && !newTks.includes(stk.tk)){
          newTks.push(stk.tk);
          sectorInfo[stk.tk] = {rank:sec.rank, sector:sec.name, reason:stk.reason, nm:stk.nm};
          // STOCKS에 없으면 추가
          if(!STOCKS.find(function(s){return s.tk===stk.tk;})){
            STOCKS.push({tk:stk.tk, nm:stk.nm, pr:0, base:1, sec:sec.name, cap:'중형'});
          }
        }
      });
    });
...
async function autoSaveJournalOnTrade(){
  ...
  const baseEntry={summary:date+' 매매 '+tt.length+'건 / '+(pnl>=0?'+':'')+pnl.toLocaleString()+'원',why_bought:'AI분석중',why_sold:'AI분석중',mistakes:'-',psychology:'-',phase_check:'-',improvement:'-',aiGenerated:false,trades:str};
  saveJEntry(date,baseEntry,pnl,wins,total,str);
  try{
    const logs=(window._decisionLog||[]).slice(-20).map(function(d){return '['+(d.phase||'')+'] '+d.title+': '+(d.body||'');}).join('\n');
    const prompt='단타 트레이딩 멘토. 오늘 매매 분석해서 매매일지 작성해줘.\n날짜: '+date+'\n매매내역:\n'+str+'\n\nAI결정로그:\n'+(logs||'없음')+'\n\n손익: '+(pnl>=0?'+':'')+pnl.toLocaleString()+'원 ('+total+'건 중 '+wins+'건 수익)\n\n강의 Phase 기준으로 평가해줘. JSON만:\n{"why_bought":"매수이유","why_sold":"청산이유","good":"잘한점","bad":"반성할점(구체적으로)","psychology":"심리평가","improvement":"개선행동","phase_check":"적용Phase와준수여부","mentor_comment":"멘토한마디(직접적으로)"}';
    ...
    const aiEntry={summary:date+' 매매 '+tt.length+'건 / '+(pnl>=0?'+':'')+pnl.toLocaleString()+'원',why_bought:ai.why_bought||'-',why_sold:ai.why_sold||'-',good:ai.good||'-',bad:ai.bad||'-',psychology:ai.psychology||'-',improvement:ai.improvement||'-',phase_check:ai.phase_check||'-',mentor_comment:ai.mentor_comment||'-',aiGenerated:true,trades:str};
