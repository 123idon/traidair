...
function registerEvolvedTechnique(gene){
  ...
  var name=gene.name||('진화기법_G'+gene.generation);
  var desc=_geneToDesc(gene);
  TECHNIQUES[name]={phase:'EVO-'+gene.generation,cond:desc,stop:'진입가 -'+gene.stopPct.toFixed(1)+'%',target:'R/R 1:'+gene.targetMult.toFixed(1),evolved:true,gene:gene,winRate:wr};
  addMsg('ai','🧬 신규 기법 등록: '+name+'\n• 조건: '+desc+'\n• 승률: '+wr+'%\n• R/R: 1:'+gene.targetMult.toFixed(1)+'\n• 세대: '+gene.generation);
  addDecisionLog('🧬 신규기법 등록',name+' — '+desc,'진화');
  return name;
}
...
function renderEvoPlusPanel(){
    ...
    if(!top5.length){topEl.innerHTML='<div style="color:var(--tm);text-align:center;padding:8px;">추가진화를 시작하세요</div>';}
      ...
      top5.forEach(function(g,i){
        ...
        if(g.fitness>=40&&wr>=45){
          html+='<button onclick="(function(){var f=_evoPlus.pool.find(function(x){return x.id===\''+g.id+'\'});if(f)registerEvolvedTechnique(f);})()" style="font-size:8px;padding:1px 4px;border:1px solid var(--g);color:var(--g);background:none;border-radius:3px;cursor:pointer;">등록</button>';
        }
        html+='</div>';
      });
  ...
  if(hallEl){
    ...
    if(!evolved.length&&!_evoPlus.hall.length){hallEl.innerHTML='<div style="color:var(--tm);font-size:9px;text-align:center;">아직 등록된 진화기법이 없습니다</div>';}
      ...
      evolved.forEach(function(k){
        var t=TECHNIQUES[k];var wr=t.winRate||0;
        html+='<div style="font-size:9px;padding:2px 0;border-bottom:1px solid var(--br);display:flex;gap:4px;align-items:center;">'+
          ...
          '<span style="font-family:var(--mono);">'+wr+'%</span>'+
        '</div>';
      });
  ...
  // 성능 변화 표시
  if(_evoPlusSnapshot.deltaFit!==undefined){
    var perf=document.getElementById('evoPlusHall');
    if(perf&&_evoPlusSnapshot.afterFitness!==undefined){
      ...
      perf.innerHTML+=
        '<div style="margin-top:6px;padding-top:5px;border-top:1px dashed var(--br);font-size:9px;">'+
        '<div style="font-weight:700;color:var(--p);margin-bottom:2px;">📈 추가진화 성능 변화</div>'+
...
async function _backtestEndOfDayInner(dt){
  ...
  try{
    if(todayTrades.length > 0){
      ...
      // AI 평가도 시도 (실패해도 위 기본 일지가 남음)
      if(typeof autoSaveJournalOnTrade === 'function'){
  ...
  try{
    ...
    if(pp.byTech){
      ...
      Object.keys(pp.byTech).forEach(function(k){
        ...
        evolveMsg+=`  ${k}: ${t.grade}등급 승률${t.winRate}% R/R${t.rr}\n`;
      });
...
async function _runScreeningAsync(){
  ...
  for(var _ti=0;_ti<allTks.length;_ti++){
    ...
    if(_tgCand.grade==='A'){score=Math.round(score*1.15);tags.push('A등급↑');}
    else if(_tgCand.grade==='D'){score=Math.round(score*0.6);tags.push('D등급↓');}
    else if(_tgCand.grade==='C'){score=Math.round(score*0.85);tags.push('C등급↓');}
...
async function execAutoBuy(pr, stk, score, techTag){
  ...
  if(_tg.grade==='A'){posPct=Math.round(posPct*1.2);addDecisionLog('['+stk.nm+'] 기법 A등급','비중 +20%','진화엔진');}
  else if(_tg.grade==='D'){posPct=Math.round(posPct*0.5);addDecisionLog('['+stk.nm+'] 기법 D등급','비중 -50%','진화엔진');}
...
위 성과 데이터를 참고해서 GO/NO-GO 판단에 반영해줘.
기법 D등급이면 진입 자제, A등급이면 비중 확대 가능.
