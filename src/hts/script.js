...
async function stopEvolvePlus(){
  ...
  var loopSummary=_evolveLoopHistory.length?'\n• 루프 이력: '+_evolveLoopHistory.map(function(h){return'L'+h.loop+'('+h.score+'점'+(h.deltaScore!==undefined?(h.deltaScore>=0?'+':'')+h.deltaScore:'')+')'}).join(' → '):'';
  addMsg('ai','🧬 진화+ 중지 (루프'+_evolveLoop+' / '+_evolveCycle+'사이클)\n• 종합점수: '+_evolveSnapshot.score+' → '+after.score+' ('+(ds>=0?'+':'')+ds+')\n• 손익: '+(_evolveSnapshot.deltaPnl>=0?'+':'')+_evolveSnapshot.deltaPnl.toLocaleString()+'원\n• 신규 매매: '+_evolveSnapshot.deltaTrades+'건\n• GA: '+_evoPlus.generation+'세대 / 등록기법: '+evolved+'개\n• 패턴DB: '+_patternDB.length+'건'+loopSummary);
}
