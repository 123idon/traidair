...
async function cleanupLearning(){
  ...
  try{
    const prompt='아래 학습 노트에서 **삭제할** 항목 번호만 알려줘.\n'+
      '삭제 기준: 완전 중복, 마크다운 헤더만 있는 줄(## 제목), 5자 미만, "전략유지/정상/양호" 같은 일반 조언.\n'+
      '⚠ 구체적 실수/개선/멘토 조언은 절대 삭제 금지. 애매하면 유지.\n\n'+
      '현재 노트:\n'+learningMemory.map((l,i)=>`[${i}] ${l.category}|${l.date}|${l.text}`).join('\n')+
      '\n\nJSON만: {"remove":[삭제할 인덱스 번호들], "reason":"정리 사유 한줄"}';
    const res=await fetch('/api/claude',{method:'POST',headers:{'Content-Type':'application/json'},
    ...
    if(!result.remove || !Array.isArray(result.remove)) throw new Error('파싱실패');
    const removeSet=new Set(result.remove);
    const before=learningMemory.length;
    if(removeSet.size > before * 0.7){
      addMsg('ai','⚠ AI가 70% 이상 삭제 시도 → 안전장치 발동. 삭제 취소.');
      if(btn){btn.textContent='AI정리';btn.disabled=false;}
      return;
    }
    learningMemory=learningMemory.filter((_,i)=>!removeSet.has(i));
    window.learningMemory=learningMemory;
    localStorage.setItem('htsLearningMemory',JSON.stringify(learningMemory));
    saveToServer('htsLearningMemory',JSON.stringify(learningMemory));
    renderLearningMgmt();
    updateLearnerStage();
    addMsg('ai','🧹 학습 노트 정리 완료\n'+before+'개 → '+learningMemory.length+'개\n'+(result.reason||''));
  }catch(e){
