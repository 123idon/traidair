...
async function _runScreeningAsync(){
  ...
  if(autoState.level>=3){
    // 학습 데이터 기반 사전 필터 — Claude 호출 전 불필요한 호출 차단
    var _preFilter=false;
    var _pfReason='';
    // (a) 패턴 DB 승률 25% 미만 → Claude 없이 PASS
    if(best.entryCond){var _pq2=queryPatternDB(best.entryCond,best.detTech);if(_pq2.count>=8&&_pq2.score<=25){_preFilter=true;_pfReason='패턴DB승률'+_pq2.score+'%('+_pq2.count+'건)';}}
    // (b) 원칙 위반 3개+ → Claude 없이 PASS
    if(best.ruleCheck&&best.ruleCheck.failed>=3){_preFilter=true;_pfReason+=((_pfReason?' + ':'')+('원칙위반'+best.ruleCheck.failed+'개'));}
    // (c) 드로다운 tier2+ → Claude 없이 규칙 기반
    if(_equityCurve.tierHit>=2){_preFilter=true;_pfReason+=((_pfReason?' + ':'')+'드로다운Lv'+_equityCurve.tierHit);}
    if(_preFilter){
      addDecisionLog('['+best.stk.nm+'] 학습필터 PASS',_pfReason,'학습차단');
      return;
    }
    // 진화+ 중 OR 점수 6 미만: 규칙 기반 판단 (Claude 스킵 — 10개 학습 시스템이 대신 판단)
    if(_evolveRunning || best.score<6){
    ...
    if(_cached&&Date.now()-_cached.ts<300000){
    ...
    try{
      ...
      var prompt='단타 자동매매 최종 진입 판단.\n'+
        '의심스러우면 PASS. 학습 데이터가 경고하면 PASS.\n\n'+
        _lec +
        _learn +
