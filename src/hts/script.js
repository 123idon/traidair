...
function computePerformanceProfile(){
  ...
  Object.keys(byTech).forEach(function(k){
    ...
    b.grade=b.winRate>=55&&parseFloat(b.rr)>=1.5?'A':b.winRate>=45&&parseFloat(b.rr)>=1.2?'B':b.winRate>=35?'C':'D';
  });
  ...
  _perfProfile={byTech:byTech,totalSells:sells.length,recentWr:recentWr,recentAvgWin:recentAvgWin,recentAvgLoss:recentAvgLoss,updated:Date.now()};
...
function computeAdaptiveConfig(){
  ...
  var avgL=p.recentAvgLoss||1;
  var avgW=p.recentAvgWin||1;
  var rr=avgL>0?avgW/avgL:1.5;
  // 적응형 손절: 승률 높으면 여유, 낮으면 타이트
  _adaptiveCfg.stop=wr>=55?3.5:wr>=45?3:2.5;
  // 적응형 목표가: R/R 기반
  ...
  // 적응형 비중: 켈리 기준 (보수적 반켈리)
  var kellyPct=(wr/100)-((1-wr/100)/(rr||1));
  var halfKelly=Math.max(10,Math.min(50,Math.round(kellyPct*50)));
  _adaptiveCfg.pos=halfKelly;
  // 적응형 R/R 최소: 승률 높으면 낮춰도 OK
  _adaptiveCfg.rr=wr>=55?1.2:wr>=45?1.5:2.0;
  // 적응형 최소 진입 점수
  _adaptiveCfg.minScore=wr>=55?5:wr>=45?6:8;
  console.log('[적응형]','wr:'+wr+'%','stop:'+_adaptiveCfg.stop+'%','pos:'+_adaptiveCfg.pos+'%','rr:'+_adaptiveCfg.rr,'minScore:'+_adaptiveCfg.minScore);
}
function getTechGrade(techName){
  if(!_perfProfile.byTech)return{grade:'?',winRate:50,adjust:1};
  var t=_perfProfile.byTech[techName];
  if(!t||t.count<3)return{grade:'?',winRate:50,adjust:1};
  var adj=t.grade==='A'?1.3:t.grade==='B'?1.0:t.grade==='C'?0.7:0.4;
  return{grade:t.grade,winRate:t.winRate,rr:t.rr,count:t.count,kelly:t.kelly,adjust:adj};
}
function buildPerfSummary(){
  ...
  Object.keys(_perfProfile.byTech).forEach(function(k){
    ...
    if(t.count<2)return;
    lines.push(k+': '+t.grade+'등급 승률'+t.winRate+'% R/R'+t.rr+' '+t.count+'건 총손익'+(t.totalPnl>=0?'+':'')+Math.round(t.totalPnl).toLocaleString()+'원');
  });
  lines.push('최근20건 승률: '+(_perfProfile.recentWr||'-')+'%');
...
var _evolveSnapshot={wr:0,pnl:0,trades:0};
...
function _takeSnapshot(){
  ...
  return{wr:total>0?Math.round(wins/total*100):0,pnl:Math.round(pnl),trades:total};
}
...
function _evolveFinish(){
  ...
  _evolveSnapshot.afterWr=after.wr;_evolveSnapshot.afterPnl=after.pnl;_evolveSnapshot.afterTrades=after.trades;
  _evolveSnapshot.deltaWr=after.wr-_evolveSnapshot.wr;
...
function renderEvolvePanel(){
  ...
  if(adEl){
    ...
    // 성능 변화 표시
    if(_evolveSnapshot.deltaWr!==undefined&&_evolveSnapshot.deltaTrades>0){
      var dwr=_evolveSnapshot.deltaWr;var dpnl=_evolveSnapshot.deltaPnl;
      var dwrCol=dwr>0?'var(--g)':dwr<0?'var(--r)':'var(--tm)';
      ...
      adEl.innerHTML+=
        ...
        '<div style="display:flex;justify-content:space-between;"><span>승률</span><b style="color:'+dwrCol+';">'+_evolveSnapshot.wr+'% → '+_evolveSnapshot.afterWr+'% ('+(dwr>=0?'+':'')+dwr+'%p)</b></div>'+
...
${_techG.grade!=='?'?`기법 과거성과: ${_techG.grade}등급 승률${_techG.winRate}% R/R${_techG.rr} ${_techG.count}건`:''}
