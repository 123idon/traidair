...
function _peekSimCandlesFor(tk, dateStr, untilIdx){
  try{
    ...
    const seed = (parseInt((dateStr||'').replace(/-/g,'').slice(-6))||9999) + tkSeed;
    const rnd = rng32(seed);
    const out = [];
    // 현재 봉 인덱스까지만 생성 (미래 봉 노출 금지)
    const limit = Math.min(perDay, (untilIdx||0)+1);
    for(let i=0;i<limit;i++){
      ...
      out.push({t:`${String(Math.floor(hm/60)).padStart(2,'0')}:${String(hm%60).padStart(2,'0')}`,date:dateStr,o:Math.round(o),h:Math.round(Math.max(o,c,h)),l:Math.round(Math.min(o,c,l)),c:Math.round(c),v});
      base=c;
    }
    return out;
  }catch(e){ return []; }
...
// STATE
// ═══════════════════════════════
let cfg={capital:10000000,dayloss:3,maxpos:50,clim:200,mlim:60,crate:8.5,mrate:12,bf:0.015,sf:0.015,tx:0.18,bd:false,al:true};
...
async function _backtestEndOfDay(){
  ...
  if(todayTrades.length > 0 && typeof autoSaveJournalOnTrade === 'function'){
    ...
    // 매매 없는 날도 "관망" 일지로 기록 → 백테스트 통계에 반영
    try{
      const noTradeEntry = {
        summary: dt+' 매매 없음 (관망)',
        why_bought:'진입 신호 없음', why_sold:'-',
        good:'필수 게이트 미달 종목 진입 회피', bad:'-',
        psychology:'-', improvement:'-', phase_check:'관망',
