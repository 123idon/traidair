...
function _evalPool(){
  ...
  _evoPlus.pool.forEach(function(gene){
    ...
    allTks.forEach(function(tk){
      var cs=null;
      try{if(window.simCandles&&simCandles[tk])cs=simCandles[tk];}catch(_e){}
      if(!cs){try{var d=sim.date||'',idx=sim.idx||0;cs=_peekSimCandlesFor(tk,d,idx);}catch(_e2){}}
      if(!cs||!cs.length){try{if(tk===activeTk)cs=getCandles(200);}catch(_e3){}}
...
function toggleEvoPlus(){
  if(_evoPlus.running){
    ...
    addMsg('ai','🧬+ 추가진화 중지. 결과 패널을 확인하세요.');
  }else{
