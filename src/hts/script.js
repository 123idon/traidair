...
function _drawChartInner(){
  ...
  try{
    ...
    _trades.forEach(function(t){
      ...
      var yPr=toY(_tpr);
      if(yPr<PT-20||yPr>PT+mainH+20){
        var cx=cs[matchIdx];
        if(cx)yPr=toY(isBuy?cx.l:cx.h);
      }
...
async function _runScreeningAsync(){
  ...
  for(var _ti=0;_ti<allTks.length;_ti++){
    ...
    // 진화: 기법 성과 기반 점수 보정 — 학습 데이터 적극 활용
    ...
    if(_tgCand.grade!=='?'){
      ...
      else if(_tgCand.grade==='D'){score=Math.round(score*0.5);tags.push('D등급↓');}
      else if(_tgCand.grade==='C'){score=Math.round(score*0.8);tags.push('C등급↓');}
      // R/R 기반 보정: 기법의 실제 R/R이 높으면 가산
      var _techRR=parseFloat(_tgCand.rr)||0;
      if(_techRR>=2){score+=2;tags.push('R/R'+_techRR);}
      else if(_techRR<0.8&&_tgCand.count>=5){score-=2;tags.push('R/R낮음');}
      // 수익팩터 기반: PF>1.5면 수익기법
      var _techPF=parseFloat(_tgCand.pf)||0;
      if(_techPF>=1.5){score+=1;tags.push('PF'+_techPF);}
      else if(_techPF<0.7&&_tgCand.count>=5){score-=1;tags.push('PF부족');}
      // 연속 손실 기법 차단: D등급 + 최근 5건 이상이면 진입 거부
      if(_tgCand.grade==='D'&&_tgCand.count>=5){
        addDecisionLog('['+stk.nm+'] '+_detTech+' D등급 차단','승률 낮은 기법 진입 거부 ('+_tgCand.count+'건 '+_tgCand.score+'점)','학습차단');
        continue;
      }
    }
    // 학습 메모리 기반 필터: 최근 학습에서 같은 기법/종목 경고가 있으면 감점
    try{
      var _lmRecent=(window.learningMemory||[]).slice(-20);
      var _lmWarn=_lmRecent.filter(function(l){
        var txt=l.text||'';
        return(txt.indexOf(_detTech)!==-1||txt.indexOf(stk.nm)!==-1)&&(l.category==='실수'||l.category==='손실'||l.category==='경고');
      });
      if(_lmWarn.length>=2){score-=3;tags.push('학습경고×'+_lmWarn.length);}
      else if(_lmWarn.length===1){score-=1;tags.push('학습주의');}
      // 성공 학습이 있으면 가산
      var _lmGood=_lmRecent.filter(function(l){
        var txt=l.text||'';
        return(txt.indexOf(_detTech)!==-1||txt.indexOf(stk.nm)!==-1)&&(l.category==='성공'||l.category==='수익');
      });
      if(_lmGood.length>=2){score+=2;tags.push('학습성공×'+_lmGood.length);}
    }catch(_lme){}
    // 전체 성과 점수 기반: 최근 성과가 나쁘면 전체적으로 보수적
    var _recentSc=_perfProfile.recentScore||50;
    if(_recentSc<30){score=Math.round(score*0.8);tags.push('성과부진');}
    else if(_recentSc>=70){score+=1;tags.push('성과우수');}
    scored.push({tk:tk,stk:stk,score:score,tags:tags,lc:lc,lma5:lma5,lrsi:lrsi,volR:volR,detTech:_detTech});
  }
...
async function execAutoBuy(pr, stk, score, techTag){
  ...
  // 기법 성과 기반 조절 — 학습 데이터 적극 반영
  var _tg=getTechGrade(techTag||'');
  if(_tg.grade==='A'){
    posPct=Math.round(posPct*1.2);
    addDecisionLog('['+stk.nm+'] 기법 A등급','비중 +20% (승률'+_tg.winRate+'% R/R'+_tg.rr+')','진화엔진');
  }else if(_tg.grade==='D'){
    posPct=Math.round(posPct*0.5);
    addDecisionLog('['+stk.nm+'] 기법 D등급','비중 -50% (승률'+_tg.winRate+'% R/R'+_tg.rr+')','진화엔진');
  }else if(_tg.grade==='C'){posPct=Math.round(posPct*0.7);}
  // 수익팩터 기반 비중 미세 조정
  if(_tg.grade!=='?'){
    var _tpf=parseFloat(_tg.pf)||1;
    if(_tpf>=2){posPct=Math.min(60,Math.round(posPct*1.1));}
    else if(_tpf<0.5&&_tg.count>=3){posPct=Math.round(posPct*0.6);}
  }
  // 전체 성과 반영: 최근 성과 점수가 낮으면 전체 비중 축소
  var _overallSc=_perfProfile.recentScore||50;
  if(_overallSc<25){posPct=Math.round(posPct*0.7);addDecisionLog('['+stk.nm+'] 전체성과 부진','비중 -30% (점수'+_overallSc+'/100)','학습반영');}
  else if(_overallSc>=75){posPct=Math.min(55,Math.round(posPct*1.1));}
  // 학습 메모리 기반: 최근 같은 종목에서 손실 학습이 있으면 비중 축소
  try{
    var _lmThis=(window.learningMemory||[]).slice(-15).filter(function(l){return(l.text||'').indexOf(stk.nm)!==-1&&(l.category==='실수'||l.category==='손실');});
    if(_lmThis.length>=2){posPct=Math.round(posPct*0.5);addDecisionLog('['+stk.nm+'] 학습 경고 반영','같은 종목 손실 학습 '+_lmThis.length+'건 → 비중 -50%','학습반영');}
    else if(_lmThis.length===1){posPct=Math.round(posPct*0.8);}
  }catch(_e){}
