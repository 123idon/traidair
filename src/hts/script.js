...
async function cleanupLearning(){
  ...
  try{
    const prompt='너는 트레이딩 멘토. 아래 학습 노트를 정리해서 **최종 버전**을 만들어줘.\n\n'+
      '정리 원칙:\n'+
      ...
      learningMemory.map((l,i)=>`- [${l.category}] ${l.text}`).join('\n')+
    ...
    if(!result.notes||!Array.isArray(result.notes)||result.notes.length<3){
      throw new Error('AI 결과 부족 ('+((result.notes||[]).length)+'개)');
    }
    const before=learningMemory.length;
    const today=typeof todayStr==='function'?todayStr():new Date().toISOString().slice(0,10);
    learningMemory=result.notes.map(n=>({
    ...
    if(learningMemory.length < Math.min(5, before)){
      ...
      addMsg('ai','⚠ AI 정리 결과가 너무 적음 ('+result.notes.length+'개) → 복원됨');
