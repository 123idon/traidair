...
function _mutate(gene){
  ...
  var r=Math.random();
  if(r<0.15){g.maAlign=['none','short','triple'][Math.floor(Math.random()*3)];}
  else if(r<0.3){g.volMin=Math.max(0.8,g.volMin+(Math.random()-0.5)*1);}
  else if(r<0.45){g.rsiLow=Math.max(15,Math.min(55,g.rsiLow+Math.floor((Math.random()-0.5)*10)));g.rsiHigh=Math.max(g.rsiLow+10,Math.min(80,g.rsiHigh+Math.floor((Math.random()-0.5)*10)));}
  else if(r<0.55){g.bodyRatio=Math.max(0.1,Math.min(0.8,g.bodyRatio+(Math.random()-0.5)*0.2));}
  else if(r<0.65){g.candleBull=!g.candleBull;}
  else if(r<0.75){g.useVwap=!g.useVwap;g.useRisingLows=!g.useRisingLows;}
  else if(r<0.85){g.stopPct=Math.max(1.5,Math.min(5,g.stopPct+(Math.random()-0.5)*1));}
...
function _testGeneOnCandles(gene,cs){
  ...
  for(var i=20;i<cs.length-5;i++){
    ...
    var lowerW=Math.min(lc.c,lc.o)-lc.l;
...
function _evalPool(){
  ...
  _evoPlus.pool.forEach(function(gene){
    ...
    allTks.forEach(function(tk){
      var cs;try{cs=window.simCandles&&simCandles[tk]?simCandles[tk]:null;}catch(_e){}
      if(!cs){try{cs=getCandles(200,tk);}catch(_e2){}}
    ...
    if(!top5.length){topEl.innerHTML='<div style="color:var(--tm);text-align:center;padding:8px;">추가진화를 시작하세요</div>';}
      ...
      top5.forEach(function(g,i){
        ...
        if(g.fitness>=40&&wr>=45){
          html+='<button onclick="registerEvolvedTechnique(_evoPlus.pool.sort(function(a,b){return b.fitness-a.fitness})['+i+'])" style="font-size:8px;padding:1px 4px;border:1px solid var(--g);color:var(--g);background:none;border-radius:3px;cursor:pointer;">등록</button>';
        }
        html+='</div>';
      });
