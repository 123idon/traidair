...
// ── API 키 만료 감지 + 어떤 API인지 추정 ──
function checkApiKeyExpiry(errorMsg, hint){
  if(!errorMsg) return;
  const msg = String(errorMsg);
  const expired = msg.includes('expired') || msg.includes('invalid') ||
  ...
  let api = hint || '';
  if(!api){
    if(/anthropic|claude|sk-ant|invalid_api_key/i.test(msg)) api = 'Claude (Anthropic)';
  ...
  // 결정 로그에도 기록
  try{ addDecisionLog('⚠ API 키 문제', api+' — '+msg.slice(0,80), 'NOGO'); }catch(_e){}
}
